...
\begin{frame}{Contesto: ottimizzazione su larga scala}
  \begin{itemize}
    ...
    \item Nei metodi classici $n_k$ è \textbf{fissa a priori}; questa tesi la
          rende \textbf{dinamica}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Metodi a varianza ridotta: l'idea comune}
  \begin{itemize}
    ...
    \item Se $c_k$ ``specchia'' il gradiente campionato (stesso esempio
      $i_k$, valutato in un punto vicino a $w_k$), sottrarla riduce la
      varianza di $g_k$, fino a zero vicino all'ottimo:
      \emph{variance reduction}. Meccanismo e forma di $c_k$ per SVRG e
      SAGA: prossima slide.
...
% =====================================================================
% SLIDE 6 — Perché la correzione riduce la varianza
% =====================================================================
\begin{frame}{Perché la correzione riduce la varianza}
  \small
  \begin{itemize}
    \item Per $w_k$ fissato, $\mathbb{E}[c_k]$ è noto: non aggiunge
      rumore. La parte aleatoria di
      $g_k=\nabla\ell(w_k;i_k)-c_k+\mathbb{E}[c_k]$ è
      $\nabla\ell(w_k;i_k)-c_k$, quindi
      \[
        \mathrm{Var}(g_k)=\mathrm{Var}\big(\nabla\ell(w_k;i_k)\big)
        +\mathrm{Var}(c_k)
        -2\,\mathrm{Cov}\big(\nabla\ell(w_k;i_k),\,c_k\big).
      \]
    \item $c_k$ è costruita usando lo \emph{stesso} esempio $i_k$ del
      gradiente: se lo ``specchia'' ($\mathrm{Cov}>0$), il termine
      $-2\,\mathrm{Cov}$ cancella parte del rumore e
      $\mathrm{Var}(g_k)$ scende. Se $c_k$ fosse indipendente non ci
      sarebbe guadagno; se anti-correlata la varianza \emph{aumenterebbe}.
    \item \textbf{SVRG}: $c_k=\nabla\ell(\bar w;i_k)$ con
      $\mathbb{E}[c_k]=\nabla J(\bar w)$. Quando $w_k\approx\bar w$, per
      la regolarità dei gradienti $c_k\approx\nabla\ell(w_k;i_k)$: la
      covarianza è quasi massima e $\mathrm{Var}(g_k)\to 0$.
    \item \textbf{SAGA}: $c_k=g^{(i_k)}$ (gradiente dell'esempio $i_k$
      memorizzato alla sua ultima occorrenza) con
      $\mathbb{E}[c_k]=\bar g$; a convergenza ($w_k\to w_*$, minimo)
      ogni voce tende a $\nabla\ell(w_*;i)$ e la cancellazione è la
      stessa di SVRG, senza ricalcoli globali.
  \end{itemize}
  \fonte{Dalla tesi --- Appendice~D (Schemi dei metodi a varianza ridotta)}
\end{frame}

% =====================================================================
% SLIDE 7 — Schema SVRG e perché ricampionare ogni m iterazioni
% =====================================================================
...
% =====================================================================
% SLIDE 9 — Limiti di SVRG/SAGA e lezione per la tesi
% =====================================================================
\begin{frame}{Limiti di SVRG/SAGA e lezione per la tesi}
  \begin{itemize}
    \item \textbf{SVRG}: una passata completa sui dati ogni $m$ iterazioni;
      se $N$ è enorme il costo dei ricalcoli periodici si fa sentire.
    ...
    \item In entrambi la dimensione del campione è \textbf{fissata a
      priori} (tipicamente un esempio per iterazione): non è mai adattata
      alle necessità di accuratezza del momento.
    \item Le garanzie di convergenza lineare valgono sotto convessità forte.
    \item \textbf{Lezione}: rendere \emph{dinamica} la dimensione del
...
% =====================================================================
% SLIDE 14 — Schema: Newton-CG (Fig. 5.2)
% =====================================================================
...
% =====================================================================
% SLIDE 15 — Schema: Newton-CG L1 (Fig. 5.5)
% =====================================================================
...
% =====================================================================
% SLIDE 16 — Schema: BB-CCV (Fig. 5.6)
% =====================================================================
...
% =====================================================================
% SLIDE 19 — Setup e risultati numerici
% =====================================================================
...
% =====================================================================
% SLIDE 20 — Riuso del mini-batch (Sez. 6.5)
% =====================================================================
