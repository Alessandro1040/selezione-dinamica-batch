...
% ---- colori (tema chiaro, sfondo bianco: nero su bianco) ----
\definecolor{sfondo}{RGB}{255,255,255}    % sfondo bianco
\definecolor{testo}{RGB}{0,0,0}           % testo nero
\definecolor{acc}{RGB}{158,108,0}         % ambra scura (titoli / kw) — leggibile su bianco
\definecolor{acc2}{RGB}{0,102,204}        % azzurro (secondario) — leggibile su bianco
\definecolor{grigio}{RGB}{110,110,110}    % note (grigio scuro)
...
% ogni formula (inline e display) in nero
...
% riquadro "take-away" / risultato chiave (fondo ambra tenue su tema chiaro)
\newcommand{\takeaway}[1]{%
  \begin{center}
    \fcolorbox{acc}{acc!12}{\parbox{0.9\textwidth}{\centering\small\kw{#1}}}
  \end{center}}
  ...
  \begin{itemize}
    ...
    \item Criterio (CCV): la varianza stimata dello stimatore del gradiente
      ...
      \[
        ...
        \qquad \theta\in(0,1)
      \]
    \item Se la CCV è violata il batch cresce per il campione successivo
      (l'iterazione corrente si completa con il campione già estratto):
      \[
        n_k^{\mathrm{new}}
        \;=\; \left\lceil
...
\begin{frame}{Schema: Dynamic GD (Fig.~5.1)}
  \begin{center}
    ...
    {\small\color{grigio} Flusso CCV: se la varianza campionaria è troppo
      grande rispetto a $\theta^2\|g_k\|^2$, $n_k$ cresce e il nuovo valore è
      usato dalla prossima estrazione; line search e aggiornamento si eseguono
      comunque con il campione corrente.}
  \end{center}
\end{frame}
...
\begin{frame}{Newton-CG $L_1$: $z_k$, active set e faccia ortante}
  ...
  \begin{itemize}\setlength{\itemsep}{1pt}
    ...
    \item $z_k$ definisce la \textbf{faccia ortante} (eq.~5.45)
      ...
      +g_k$ (eq.~5.43), con $g_k$ il subgradiente scelto dalla regola di
      proiezione: $g_k^i=\nu z_k^i$ se $z_k^i\neq0$, mentre per le variabili
      bloccate ($z_k^i=0$) la componente di $\widetilde{\nabla}F$ è nulla; su
      $\Omega_k$ si minimizza il modello quadratico
