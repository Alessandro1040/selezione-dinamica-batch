...
% =====================================================================
% SLIDE 13 — Newton-CG: criterio di arresto adattivo del CG
% =====================================================================
\begin{frame}{Newton-CG: criterio di arresto adattivo del CG}
  \small
  \begin{itemize}
    \item Il sistema di Newton è risolto in modo Hessian-free con il CG,
      usando per l'Hessiana il sottocampione
      $\mathcal{H}_k\subseteq\mathcal{S}_k$:
      \[
        \nabla^2 J_{\mathcal{H}_k}(w_k)\,d=-\nabla J_{\mathcal{S}_k}(w_k),
        \qquad
        r_k=\nabla^2 J_{\mathcal{H}_k}(w_k)\,d+\nabla J_{\mathcal{S}_k}(w_k).
      \]
      L'errore di Hessiana $\Delta_{\mathcal{H}_k}(w_k;d)$ (differenza tra
      le Hessiane sui campioni $\mathcal{S}_k$ e $\mathcal{H}_k$ lungo $d$)
      è costante e non calcolabile: non serve spingere $r_k$ a zero, basta
      fermarsi quando è dello \emph{stesso ordine} di $\Delta$.
    \item L'errore si stima con la \emph{varianza campionaria dei prodotti
      Hessiana-vettore} su $\mathcal{H}_k$ (stessa strategia usata per il
      gradiente):
      \[
    ...
    \item Per l'omogeneità quadratica in $d$, la varianza è calcolata una
      sola volta sulla prima direzione del CG $p_0=-g_k$; la soglia cresce
      con $\|d\|^2$ (evita anche direzioni troppo grandi):
      \[
        \gamma=\frac{\bigl\|\mathrm{Var}_{i\in\mathcal{H}_k}
          \bigl(\nabla^2\ell(w_k;i)\,p_0\bigr)\bigr\|_1}
          {|\mathcal{H}_k|\,\|p_0\|_2^2},
        \qquad
        \Psi(d)=\gamma\,\|d\|_2^2.
      \]
    \item \textbf{Test di arresto} del CG (a ogni iterazione $j$):
      \[
