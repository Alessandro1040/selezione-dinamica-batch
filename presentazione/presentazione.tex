...
\begin{frame}{Perché il riuso è critico sui mal condizionati: l'esempio $\kappa$}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Primo ordine ($d_k=-g_k$): deriva permanente.} Il gradiente
    ...
    \item \textbf{Secondo ordine ($d_k=-H_k^{-1}g_k$): Hessiana obsoleta.}
...
\begin{frame}{Validation set: perché evita il collasso di $M{=}\infty$}
  ...
  \setlength{\belowdisplayshortskip}{2pt}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Perché $M{=}\infty$ collassa (Newton).} $H_k$ è calcolata nel
    ...
    \item \textbf{La validation è un giudice esterno} (i suoi dati non entrano
      ...
      ``rinfrescata''.
    \item \textbf{seed 42 (Tab.~6.3).} Newton-CG su $\kappa\approx1.1$:
    ...
    \item \textbf{5 seed.} Default $2.67\times10^{-1}$ (7/20), peggiore della
      base ($2.16\times10^{-1}$); calib. $P{=}1,p{=}10\%$,dyn $2.04\times10^{-1}$
      (12/20), sotto base e molto sotto $M{=}\infty$ ($3.84\times10^{-1}$).
    \item \textbf{Perché $P{=}1$, $p{=}10\%$, dyn.} Pazienza bassa = criterio
...
\begin{frame}{Discesa della loss sul batch: lettura dei risultati}
  ...
  \setlength{\belowdisplayshortskip}{2pt}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Meccanismo (ricorda).} Nessun dato riservato: si campiona
    ...
    \item \textbf{Newton protetto.} Newton-CG su $\kappa\approx20$: da $1.28$
      ($M{=}\infty$) a $2.48\times10^{-1}$; evita il collasso anche per
      Newton-CG $L_1$ (si monitora $F_{\mathrm{batch}}$).
    \item \textbf{Punto debole: Rosenbrock.} Sul batch locale la loss scende
    ...
    \item \textbf{5 seed (Tab.~6.7).} Discesa calib.
    ...
    \item \textbf{In sintesi.} È l'alternativa alla validation \emph{senza
