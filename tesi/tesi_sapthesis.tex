...
\begin{tikzpicture}[
  ...
  io/.style={
    draw=gray!60, fill=gray!8, rounded corners=3pt,
    text width=4.2cm, align=center, minimum height=1.0cm,
    inner sep=4pt
  },
  arrow/.style={-{Stealth[length=2.5mm]}, thick, color=coverblue}
]

  % Colonna sinistra: preparazione dati
  \node[io] (data) {Dati: clip audio\\[2pt] (addestramento e test)};
  \node[box, below=of data] (feat) {Rappresentazione e normalizzazione\\[2pt] ogni clip $\to$ vettore $x_i \in \mathbb{R}^d$};

  % Colonna destra: modello e algoritmi
  \node[box, right=2.0cm of data] (net) {Definizione della rete\\[2pt] parametri $w = \mathrm{vec}(W_l, b_l)$};
  \node[box, below=of net] (algs) {Esecuzione dei quattro algoritmi\\[2pt] (listati B.1--B.4)};
