...
% --- Box personalizzati (padding leggermente compatto: serve per non sforare pagine) ---
\tcbset{boxsep=1mm, top=1mm, bottom=1mm}
\newtcolorbox{notebox}{
...
\newcommand{\norm}[1]{\left\|#1\right\|}
\newcommand{\grad}{\nabla}
% --- Dati del frontespizio sapthesis (pagina 1, come frontespizio.tex) ---
...
% La pagina del verso NON è generata (come in frontespizio.tex)
\makeatletter
\renewcommand{\SAP@composebacktitlepage}{}
\makeatother

% Numerazione e TOC coerenti con la vecchia struttura article (section->chapter)
\setcounter{secnumdepth}{2}
\setcounter{tocdepth}{2}

% --- Impaginazione a flusso continuo (vicina alla versione article): niente
% pagine bianche (\cleardoublepage = \clearpage) e capitoli senza salto di
% pagina forzato (si va a capo solo quando il contenuto non entra).
\makeatletter
\renewcommand{\cleardoublepage}{\clearpage}
\let\SAP@orig@chapter\chapter
\renewcommand{\chapter}{\thispagestyle{plain}\global\@topnum\z@\@afterindentfalse\secdef\@chapter\@schapter}
% Titoli di capitolo piu' compatti (proporzionati alle vecchie \section)
...
% ── PRELIMINARI (sapthesis: frontespizio, ringraziamenti, abstract) ─────
\frontmatter
\maketitle
...
\newpage
\hypersetup{linkcolor=black}% indice senza link blu
\tableofcontents
\hypersetup{linkcolor=coverblue}%
\mainmatter
...
\section{Il problema: la dimensione del mini batch}
...
\section{La soluzione proposta: il campionamento dinamico}
...
% ═══════════════════════════════════════════════════════════════════════════
\chapter{Background e Notazione}\label{sec:background}
% ═══════════════════════════════════════════════════════════════════════════

Questo capitolo richiama la notazione usata nel resto del lavoro.

\section{Glossario dei simboli ricorrenti}
...
$\kappa = L/\lambda$ & sovrastima del numero di condizionamento (che, in norma 2 e per matrici simmetriche, è $\lambda_{\max}/\lambda_{\min}$; cfr. Cap.~\ref{sec:formulazione}) \\
...
\begin{notebox}
  ...
  (CCV) introdotta nel Capitolo~\ref{sec:algoritmi}.
\end{notebox}


\section{Collegamento con la formulazione del problema}
...
% ═══════════════════════════════════════════════════════════════════
\chapter{Formulazione del Problema}\label{sec:formulazione}
% ═══════════════════════════════════════════════════════════════════════════

\section{Apprendimento supervisionato su larga scala}
...
% ═══════════════════════════════════════════════════════════════════════════
% ═══════════════════════════════════════════════════════════════════════════
\chapter{Lavori Correlati}\label{sec:lavori}
% ═══════════════════════════════════════════════════════════════════════════
...
\section{Metodi di ottimizzazione stocastica}
...
\section{Campionamento dinamico}
...
\section{Metodi di secondo ordine stocastici}
...
\section{Regolarizzazione $L_1$ in ottimizzazione}
...
\chapter{Algoritmi Proposti}\label{sec:algoritmi}
% ═══════════════════════════════════════════════════════════════════════════

\section{Metodo del Gradiente a Campione Dinamico}


\subsection{Formulazione della dinamica del batch}
...
\subsection{Formulazione matematica della condizione di accettazione}
...
\end{thmbox}
\enlargethispage{5\baselineskip}% spazio extra per far entrare anche il notebox successivo
\begin{notebox}
...
\subsection{Regola di aggiornamento del batch}
...
\subsection{Pseudocodice (Algoritmo)}
...
\newpage
\subsection{Convergenza deterministica}\label{sec:convergenza-deterministica}
...
\subsection{Analisi Stocastica e Complessità del Metodo}
...
\section{Metodo di Newton-CG con Campionamento Dinamico}
...
\subsection{Struttura del metodo}
...
\subsection{Criterio di terminazione del gradiente coniugato}
...
\medskip
\noindent
\subsection{Pseudocodice (Algoritmo Newton-CG)}
...
\section{Metodo Newton-CG per Modelli con Regolarizzazione $L_1$}\label{sec:l1}
...
\subsection{Il gradiente generalizzato}
...
\subsection{Identificazione dell'Active Set e della Faccia Ortante}
...
\subsection{Fase di Minimizzazione nel Sottospazio}
...
\subsection{Ricerca Lineare Proiettata}
...
\chapter{Esperimenti e Visualizzazione Interattiva}\label{sec:esperimenti}
% ═══════════════════════════════════════════════════════════════════════════
...
\section{Architettura Software}\label{sec:architettura}
...
\section{Risultati Numerici}\label{sec:risultati}
...
\section{Visualizzazione Interattiva}\label{sec:visualizzazione}
...
\section{Validazione su un benchmark reale: NSynth}\label{sec:nsynth}
...
\subsection{Estensione oltre le ipotesi: implementazione vettorizzata per la rete neurale}\label{sec:nsynth-net}
...
dell'ordine di $10^{-15}$). La Figura~\ref{fig:nn_pipeline} mostra la pipeline
sperimentale.
...
\begin{tikzpicture}[
  ...
  box/.style={
    draw=coverblue, fill=coverblue!6, rounded corners=3pt,
    text width=4.6cm, align=center, minimum height=1.2cm,
    inner sep=6pt
  },
  io/.style={
    draw=gray!60, fill=gray!8, rounded corners=3pt,
  ...
  \node[io] (data) {Dati: clip audio\\[2pt] (addestramento e test)};
  \node[box, below=of data] (feat) {Rappresentazione di ogni clip\\[2pt] $x_i \in \mathbb{R}^d$\\[1pt] (descrittori spettrali e temporali)};
  \node[box, below=of feat] (norm) {Normalizzazione dei dati\\[2pt] (mediana e dispersione uniformi)};
  \node[box, below=of norm] (net) {Definizione della rete\\[2pt] $w = \mathrm{vec}(W_1, b_1, W_2, b_2, W_3, b_3)$};
  \node[box, below=of net] (algs) {Esecuzione dei quattro algoritmi\\[2pt] (listati B.1--B.4) con funzioni batch};
  \node[io, below=of algs] (eval) {Valutazione su test\\[2pt] accuratezza, figure, tabelle};

  \draw[arrow] (data) -- (feat);
  \draw[arrow] (feat) -- (norm);
  \draw[arrow] (norm) -- (net);
  \draw[arrow] (net) -- (algs);
  \draw[arrow] (algs) -- (eval);

\end{tikzpicture}}
\caption{Pipeline sperimentale per la rete neurale: ogni clip audio è
rappresentata da un vettore $x_i$ di $d$ descrittori che riassumono la
distribuzione dell'energia del segnale nel tempo e in frequenza; i dati sono
poi normalizzati e passati alla rete, su cui vengono eseguiti gli algoritmi
dei listati B.1--B.4 con le funzioni batch vettorizzate.}
\label{fig:nn_pipeline}
\end{figure}

\textbf{La rete neurale.} La rete è una funzione $f_w$ che trasforma il vettore
$x$ in un vettore di punteggi per le 10 famiglie strumentali, alternando
trasformazioni affini e attivazioni non lineari $\tanh$:
$z_1 = xW_1 + b_1$, $a_1 = \tanh(z_1)$, $z_2 = a_1 W_2 + b_2$,
$a_2 = \tanh(z_2)$, $\mathrm{logits} = a_2 W_3 + b_3$. L'ultima trasformazione
produce i \emph{logits}, convertiti in probabilità dalla softmax. La loss è la
cross-entropia (il logaritmo negativo della probabilità assegnata alla classe
corretta) più un termine di regolarizzazione che penalizza i pesi di grande
modulo e favorisce soluzioni stabili. La Figura~\ref{fig:nn_arch} mostra il
flusso dei dati nella rete.
...
\begin{tikzpicture}[
  ...
  \node[io] (x) {Input\\[2pt] $x \in \mathbb{R}^d$};
  \node[box, right=of x] (l1) {Strato 1\\[2pt] $z_1 = xW_1 + b_1$\\[1pt] $a_1 = \tanh(z_1)$};
  \node[box, right=of l1] (l2) {Strato 2\\[2pt] $z_2 = a_1 W_2 + b_2$\\[1pt] $a_2 = \tanh(z_2)$};
  \node[box, right=of l2] (l3) {Strato 3\\[2pt] $\mathrm{logits} = a_2 W_3 + b_3$};
  \node[box, right=of l3] (sm) {Softmax\\[2pt] $p_c = \frac{e^{z_c}}{\sum_{c'} e^{z_{c'}}}$};
  \node[io, right=of sm] (loss) {Loss\\[2pt] $-\log p_y + \frac{\lambda}{2}\|W\|^2$};
...
\end{tikzpicture}}
\caption{Architettura della rete neurale: tre trasformazioni affini (con
$W_l$ e $b_l$ i pesi e i bias dello strato $l$) intervallate dall'attivazione
$\tanh$; l'ultimo strato produce i logits, la softmax le probabilità sulle
classi e la loss misura l'errore sulla previsione più la regolarizzazione.}
\label{fig:nn_arch}
\end{figure}

\textbf{Gradiente del batch.} Per minimizzare la loss con gli algoritmi della
tesi serve il gradiente rispetto ai parametri. Lo si calcola con la
\textbf{retropropagazione}: dopo il forward si definisce l'errore
$\delta = (P - Y)/|\mathcal{S}|$ tra le probabilità previste $P$ e la codifica
one-hot $Y$ delle classi vere, e lo si propaga all'indietro attraverso i tre
strati usando la derivata di $\tanh$, che vale $1 - a^2$; a ogni peso si
aggiunge il contributo di regolarizzazione $\lambda W_l$. Il risultato è la
...
\begin{tikzpicture}[
  ...
  \node[io] (in) {Batch $\mathcal{S}$\\[2pt] (feature e label)};
  \node[box, below=of in] (fwd) {Forward su tutto il batch\\[2pt] attivazioni $a_1, a_2$, probabilità $P$};
  \node[box, below=of fwd] (err) {Errore\\[2pt] $\delta = (P - Y)/|\mathcal{S}|$};
  \node[box, below=of err] (bp) {Retropropagazione\\[2pt] $\delta_2 = (\delta W_3^{\top}) \odot (1-a_2^2)$\\[1pt] $\delta_1 = (\delta_2 W_2^{\top}) \odot (1-a_1^2)$};
  \node[box, below=of bp] (reg) {Gradienti dei pesi\\[2pt] $\nabla_{W_l} + \lambda W_l$, \quad $l = 1,2,3$};
  \node[io, below=of reg] (out) {$\nabla J_\mathcal{S}(w)$};
...
\end{tikzpicture}}
\caption{Calcolo vettorizzato del gradiente sul batch (retropropagazione):
il forward produce le probabilità, l'errore $\delta$ viene propagato
all'indietro attraverso i tre strati e si somma la regolarizzazione; il
risultato coincide con la media dei gradienti per-esempio.}
\label{fig:nn_grad}
\end{figure}

\textbf{Varianza per-esempio per la CCV.} La condizione di controllo della
varianza (listato B.1) richiede la varianza campionaria dei gradienti
individuali sul batch corrente. In forma esatta essa vale
$V = \frac{\sum_i\|g_i\|^2 - n_k\|\bar g\|^2}{n_k - 1}$: per calcolarla non
serve materializzare la matrice $(n_k \times D)$ dei gradienti individuali
...
gradienti a blocchi di 32 esempi. Il valore ottenuto è \emph{identico} a
quello del listato B.1. La Figura~\ref{fig:nn_ccv} riassume il procedimento.
...
\begin{tikzpicture}[
  ...
  \node[io] (in) {Batch $\mathcal{S}$\\[2pt] di dimensione $n_k$};
  \node[box, below=of in] (mean) {Gradiente medio\\[2pt] $\bar g = \texttt{grad\_batch}(w, \mathcal{S})$};
  \node[box, below=of mean] (norms) {Norme per-esempio\\[2pt] per blocchi di 32: accumula $\sum_i\|g_i\|^2$};
  \node[box, below=of norms] (var) {Varianza\\[2pt] $V = \dfrac{\sum_i\|g_i\|^2 - n_k\|\bar g\|^2}{n_k - 1}$};
  \node[io, below=of var] (out) {$(V, \|\bar g\|^2)$\\[2pt] per la verifica CCV};

  \draw[arrow] (in) -- (mean);
  \draw[arrow] (mean) -- (norms);
  \draw[arrow] (norms) -- (var);
  \draw[arrow] (var) -- (out);

\end{tikzpicture}}
\caption{Calcolo della varianza per-esempio per la CCV: si ottiene la media
$\bar g$ del gradiente sul batch, si accumula la somma dei quadrati delle norme
dei gradienti individuali a blocchi (per limitare la memoria) e si combina il
tutto nella formula esatta; il valore coincide con quello del listato B.1.}
\label{fig:nn_ccv}
\end{figure}

\textbf{Prodotto Hessiano--vettore.} Il metodo Newton-CG richiede, a ogni
iterazione del gradiente coniugato, il prodotto tra l'Hessiana della loss media
sul sottocampione $\mathcal{H}_k$ e un vettore $v$. Poiché l'Hessiana della
media è la media delle Hessiane, questo prodotto coincide con la media dei
prodotti per-esempio; viene calcolato con un'unica chiamata di
differenziazione automatica sulla funzione $f(w') = J_{\mathcal{H}_k}(w')$
(Figura~\ref{fig:nn_hess}). Per contenere il costo si pone $\gamma = 0$ nel
criterio di arresto del CG (la stima della varianza degli Hessiani per-esempio
sarebbe proibitiva) e si usa $\alpha = 1$ come passo iniziale delle line
search, come nel resto della tesi.
...
\begin{tikzpicture}[
  ...
  \node[io] (in) {Sottocampione $\mathcal{H} \subseteq \mathcal{S}$\\[2pt] e vettore $v$};
  \node[box, below=of in] (f) {Loss media sul sottocampione\\[2pt] $f(w') = J_\mathcal{H}(w')$};
  \node[box, below=of f] (hvp) {Differenziazione automatica\\[2pt] $\nabla^2 f(w)\,v$ (una sola chiamata)};
  \node[io, below=of hvp] (out) {Vettore per il gradiente coniugato};
...
\end{tikzpicture}}
\caption{Prodotto Hessiano--vettore sul sottocampione: si definisce la loss
media $f$ sul batch Hessiano e se ne calcola $\nabla^2 f(w)\,v$ in un'unica
chiamata di differenziazione automatica, ottenendo lo stesso valore della media
dei prodotti per-esempio.}
\label{fig:nn_hess}
\end{figure}

Il notebook \texttt{nsynth\_net\_riproduzione}, disponibile nella cartella
\texttt{tesi/nsynth/} del repository, implementa fedelmente gli schemi qui
descritti: la cella di validazione interna verifica numericamente, su un
...
% ═══════════════════════════════════════════════════════════════════════════
\chapter{Conclusioni e Lavoro Futuro}\label{sec:conclusioni}
% ═══════════════════════════════════════════════════════════════════════════

\section{Riassunto dei risultati}
...
\section{Direzioni per lavoro futuro}
...
\appendix
\chapter{Dimostrazioni}\label{app:dimostrazioni}

\section{Dimostrazione delle disuguaglianze di \newline Polyak--Lojasiewicz}\label{app:pl}
...
\section{Dimostrazione del fattore di correzione per \newline popolazione finita}\label{app:corr}
...
\section{Dimostrazione delle formule per $\alpha_k$ e $\beta_k$ nel Metodo del Gradiente Coniugato}
\label{app:cg}
...
\newpage
\section{Spiegazione delle condizioni sul passo $\alpha_k$}\label{app:wolfe}
\textit{
...
% ═══════════════════════════════════════════════════════════════════════════
% ═══════════════════════════════════════════════════════════════════════════
\chapter{Listati Python completi}\label{app:listati}
% ═══════════════════════════════════════════════════════════════════════════
...
\section{B.1: metodo del gradiente a campione dinamico}
...
\section{B.2: Newton-CG con campionamento dinamico}
...
\section{B.3: Newton-CG per problemi $L_1$-regolarizzati}
...
\section{B.4: metodo BB-CCV (Barzilai--Borwein con campionamento dinamico)}
...
\chapter{Istruzioni per l'esecuzione}\label{app:istruzioni}
% ═══════════════════════════════════════════════════════════════════════════
...
\section{Esecuzione degli esperimenti numerici}
...
\section{Esecuzione dei singoli algoritmi}
...
\section{Avvio dell'applicazione web}
...
% ═══════════════════════════════════════════════════════════════════════════
\chapter{Schemi dei metodi a varianza ridotta}\label{app:schemi}
% ═══════════════════════════════════════════════════════════════════════════

I metodi a varianza ridotta SVRG e SAGA, discussi nel
Capitolo~\ref{sec:lavori}, combinano la stima stocastica del gradiente con una
...
% ═══════════════════════════════════════════════════════════════════════════
\chapter{Schema dell'algoritmo BB-CCV}\label{app:bbccv}
% ═══════════════════════════════════════════════════════════════════════════
...
\section{Il metodo di Barzilai--Borwein}\label{app:bb}
...
richiedere la Hessiana. Questa sottosezione illustra da dove nasce la formula
del passo e perché il metodo è particolarmente efficace sulle funzioni
quadratiche.

Consideriamo due iterazioni
consecutive, il punto corrente $w_k$ e il successivo $w_{k+1} = w_k + \Delta
w_k$. Lo sviluppo al primo ordine del gradiente $g(w) = \nabla J(w)$ attorno a
$w_k$ è
\[
g(w_{k+1}) = g(w_k + \Delta w_k) \approx g(w_k) + \nabla^2 J(w_k)\, \Delta
w_k, \tag{E.1}
\]
dove $\nabla^2 J(w_k)$ è la Hessiana (vera) di $J$ nel punto $w_k$.

Per rendere la scrittura più
compatta definiamo lo \emph{spostamento} compiuto e la \emph{variazione
osservata del gradiente}:
\[
...
Sostituendo le
definizioni (E.2) nell'espansione (E.1) otteniamo
\[
y_k \approx \nabla^2 J(w_k)\, s_k, \tag{E.3}
\]
la \emph{relazione fondamentale}: la Hessiana vera, applicata allo spostamento
compiuto, produce la variazione osservata del gradiente. Nel passo usato da
BB-CCV la coppia $(s_k, y_k)$ è riferita all'iterazione precedente,
$s_k = w_k - w_{k-1}$ e $y_k = g_k - g_{k-1}$ (Figura~\ref{fig:bbccv}).

Calcolare esplicitamente
$\nabla^2 J(w_k)$ è in generale proibitivo. I metodi quasi Newton aggirano il
problema costruendo un'approssimazione $B_{k+1}$ della Hessiana e imponendo
che si comporti come la Hessiana vera sull'ultimo spostamento osservato:
\[
B_{k+1}\, s_k = y_k. \tag{E.4}
\]
Questa condizione, nota come \emph{equazione della secante}, è il punto di
partenza di tutti i metodi quasi Newton: per determinare $B_{k+1}$ in modo
unico essi aggiungono altre condizioni (simmetria, definitezza positiva,
minima distanza dall'approssimazione precedente), ottenendo una matrice piena
$n \times n$ aggiornata a ogni passo. Il metodo di Barzilai--Borwein parte
dalla stessa equazione ma fa una scelta di modellazione molto più estrema:
approssima l'intera Hessiana con la matrice scalare
$B_{k+1} = \alpha_{k+1}^{-1} I$, cioè con un unico numero $\alpha_{k+1}$. Con
questa ipotesi la (E.4) diventa
\[
\alpha_{k+1}^{-1}\, s_k \approx y_k,
\]
un sistema di $n$ equazioni scalari nell'incognita $\alpha_{k+1}$: in
generale nessun singolo scalare può soddisfarle esattamente tutte, quindi il
problema si risolve ai minimi quadrati.

Dalla relazione
$\alpha^{-1} s \approx y$ si ottengono due formule classiche. La prima,
minimizzando $\|\alpha y - s\|_2^2$, dà
\[
\alpha_k^{(1)} = \frac{s_k^\top y_k}{y_k^\top y_k}; \tag{E.5}
\]
la seconda, minimizzando $\|\alpha^{-1} s - y\|_2^2$, dà
\[
\alpha_k^{(2)} = \frac{s_k^\top s_k}{s_k^\top y_k}. \tag{E.6}
\]
La seconda è quella usata in BB-CCV (Figura~\ref{fig:bbccv}): è più
aggressiva, adatta la curvatura in modo più rapido ed è l'inverso del
quoziente di Rayleigh della Hessiana lungo $s_k$ (per una quadratica con
Hessiana $A$, $s^\top y = s^\top A s$ e $\alpha^{(2)}$ è l'inverso della
curvatura media di $A$ lungo la direzione percorsa, cioè un passo di Newton
scalare a costo nullo). Può però esplodere quando $s$ e $y$ sono quasi
ortogonali, da cui la salvaguardia $\mathrm{clip}(\cdot, \alpha/20, 5\alpha)$
di BB-CCV.

A differenza degli altri metodi
quasi Newton, che aggiornano l'approssimazione $B_k$ tenendo conto di tutta
la storia passata (più passi si fanno, più precisa diventa), il metodo di
Barzilai--Borwein ricalcola $\alpha$ a ogni iterazione usando soltanto gli
ultimi due punti. Per una quadratica questo non è un problema, perché la
curvatura è invariante; per una funzione generica significa ignorare come sta
cambiando la forma della funzione.

L'approssimazione
$B = \alpha^{-1} I$ è esatta solo quando la Hessiana è costante in tutto lo
spazio, cioè quando $J$ è quadratica. In quel caso $y = A s$ vale esattamente
e globalmente, e l'informazione sulla curvatura imparata tra $w_{k-1}$ e
$w_k$ vale perfettamente per il passo successivo. Inoltre, se $A$ è definita
positiva, $s^\top y = s^\top A s > 0$: il denominatore è sempre positivo, il
passo è sempre ben definito e, per la disuguaglianza di Cauchy‑Schwarz, cade
in un intervallo ragionevole. Il passo $\alpha^{(2)}$ è esattamente il
minimizzatore di $J$ lungo la direzione percorsa, e il metodo converge in
pochissime iterazioni.

Quando $J$ non è quadratica
le proprietà precedenti vengono meno, per quattro motivi collegati.
\begin{enumerate}
  \item \emph{L'equazione della secante è vera solo localmente.} Per una
  funzione generica la variazione del gradiente è legata allo spostamento dal
  Teorema Fondamentale del Calcolo:
  \[
  y_{k-1} = \int_0^1 \nabla^2 J(w_{k-1} + t s_{k-1})\, s_{k-1}\, dt,
  \]
  cioè un integrale della Hessiana lungo tutto il segmento percorso. Il metodo
  prende questo valore medio e lo usa per stimare $\alpha$: nel caso
  quadratico l'integrale si riduce a $A s_{k-1}$ e l'informazione vale
  ovunque; nel caso generale la curvatura cambia da punto a punto e il passo
  calcolato descrive la curvatura \emph{passata}. Nel nuovo punto $w_{k+1}$ la
  curvatura locale può essere molto diversa: è come guidare guardando lo
  specchietto retrovisore, la strada dietro è dritta ma davanti c'è una curva.
  \item \emph{Il metodo è privo di memoria.} Poiché $\alpha$ è ricalcolato dai
  soli ultimi due punti, il metodo reagisce in modo impulsivo all'ultima
  oscillazione, senza costruire un'immagine stabile della funzione: più la
  forma della funzione cambia, più questa mancanza di memoria è costosa.
  \item \emph{Il denominatore $s^\top y$ perde le sue garanzie.} Per una
  funzione non quadratica la Hessiana non è definita positiva ovunque. Se
  $s^\top y < 0$ (curvatura negativa), la formula $\alpha = \|s\|_2^2 /
  (s^\top y)$ produce un passo negativo, cioè di salita: la salvaguardia di
  BB-CCV lo blocca ad $\alpha/20$, ma a quel punto si usa un passo fisso, non
  più il passo di Barzilai--Borwein. Se $s^\top y \approx 0$ (Hessiana quasi
  singolare nella direzione percorsa), il passo diverge e la salvaguardia a
  $5\alpha$ impedisce il collasso dell'iterazione.
  \item \emph{Le derivate terze distruggono l'ottimalità.} Sulle quadratiche
  la direzione di discesa e la curvatura sono allineate e
  $\alpha = \|s\|_2^2/(s^\top A s)$ è esattamente il passo che minimizza $J$
  lungo la retta percorsa (minimizzazione esatta). Sulle funzioni generali il
  minimo lungo la retta non si trova più con quella formula, perché compaiono
  termini cubici, quartici e così via: il passo di Barzilai--Borwein
  minimizza l'errore sulla secante, non la funzione, quindi non è mai
  perfettamente allineato con la curvatura locale e l'algoritmo compie più
  oscillazioni.
\end{enumerate}

Il passo di Barzilai--Borwein è la soluzione analitica
esatta del problema di minimizzazione lungo la direzione percorsa nel caso
quadratico; sulle funzioni generali è invece un'approssimazione euristica
della curvatura media del tratto appena percorso, tanto più affidabile quanto
più lentamente cambia la Hessiana, cioè quanto più piccole sono le derivate
terze. Per problemi fortemente non lineari, come le reti neurali profonde, il
passo BB puro è raramente utilizzato senza salvaguardie (il clipping e la
line search di Armijo di BB-CCV) o senza essere integrato in metodi più
robusti, come i metodi a varianza ridotta dell'Appendice~\ref{app:schemi}.
Sui problemi della Sezione~\ref{sec:risultati} la salvaguardia è sufficiente:
BB-CCV è il metodo più veloce e raggiunge la precisione macchina sui problemi
mal condizionati.
...
\section{Schema dell'algoritmo BB-CCV}\label{app:bbccv-schema}
