...
\end{center}
\noindent
Questo documento presenta, in forma rigorosa ma accessibile, la metodologia per la selezione dinamica della dimensione del campione negli algoritmi di ottimizzazione per il machine learning su larga scala, basandosi sul lavoro seminale di Byrd, Chin, Nocedal e Wu (2012). Il filo conduttore \`e il seguente: anzich\'e scegliere a priori una dimensione fissa del batch (il sottoinsieme di dati usato ad ogni iterazione), \`e possibile decidere durante l'algoritmo se e quando aumentarla, guidandosi da stime statistiche della varianza del gradiente. Vengono trattati tre contributi principali: (i) un metodo del gradiente a campione dinamico con analisi di complessit\`a, (ii) un metodo di Newton con gradiente coniugato e Hessiana sottocampionata, (iii) un'estensione ai problemi con regolarizzazione $L_1$ per la produzione di soluzioni sparse. Il documento \`e strutturato per essere comprensibile anche a chi non ha una formazione avanzata in ottimizzazione, introducendo gradualmente i concetti e corredando ogni risultato teorico con un'interpretazione intuitiva.
\begin{center}
...
\begin{definitionbox}
  Funzione convessa. Una funzione $f: \R^m \to \R$ si dice convessa se, preso qualunque segmento tra due punti del suo dominio, il grafico della funzione sta sotto (o al pi\`u su) quel segmento. Formalmente, per ogni $x, y \in \R^m$ e $t \in [0,1]$:
  \[
    f(tx + (1-t)y) \leq t f(x) + (1-t) f(y).
  \]
  L'utilit\`a delle funzioni convesse in ottimizzazione \`e fondamentale: ogni minimo locale \`e anche globale.

\end{definitionbox}

\begin{definitionbox}
  Gradiente. Il gradiente $\nabla f(w) \in \R^m$ di una funzione differenziabile $f: \R^m \to \R$ nel punto $w$ \`e il vettore delle derivate parziali:
  \[
    \nabla f(w) = \left(\frac{\partial f}{\partial w_1}, \frac{\partial f}{\partial w_2}, \dots, \frac{\partial f}{\partial w_m}\right)^T.
  \]
  Punta nella direzione di massima crescita di $f$; pertanto $-\nabla f(w)$ \`e la direzione di massima discesa, usata negli algoritmi di ottimizzazione.
  
\end{definitionbox}


\begin{definitionbox}
  Norma di una matrice. Data una matrice $A \in \R^{n \times n}$, la norma indotta (o norma operatoriale) è definita come:
  \[
 ...
 \begin{mnbox}
  Numero di condizionamento e convergenza.
  ...
  positiva, il numero di condizionamento è definito come
  \[
  \kappa(A) = \frac{\lambda_{\max}(A)}{\lambda_{\min}(A)}.
  \]
  Questo numero misura la sensibilità del problema alla propagazione
  ...
  $\kappa(A)$ grande si dice mal condizionato, perché la soluzione è
...
\begin{comment}
\begin{notebox}
  Nota sul significato di $\mathcal{V}$
...
Vogliamo misurare quanto sono diversi tra loro questi $N$ vettori: $\mathcal{V}$ è  la media degli scarti quadratici di ogni singolo vettore $\nabla \ell(w_k; i)$ dalla loro media $\nabla J(w_k)$:
  ...
  \]
  dove il quadrato è inteso componente per componente.
  
In termini probabilistici, se estrai a caso un indice $I$ dal dataset, il gradiente della loss su quell'esempio casuale è una variabile aleatoria. $\mathcal{V}$ è la varianza (componente per componente) di questa variabile aleatoria intorno alla sua media, cioè:
\[
...
\begin{notebox}
  Confronto con il risultato di Nocedal (Theorem 4.1).
...
\begin{itemize}
    ...
    \item Ipotesi stocastica (3.4): garantisce che la varianza dello stimatore \(g_k\) decada come \(\|\mathrm{Var}(g_k)\|_1 \le \omega/n_k\), rendendo il rumore controllabile aumentando il batch.
\end{itemize}
...
\begin{thmbox}
Teorema 3.1 (Convergenza in aspettativa).
...
\begin{notebox}
  Differenza rispetto al paper (Byrd et al. 2012, Teorema 4.2).
...
\begin{corollarybox}
  Corollario 3.2 (Complessità totale).
...
Per evitare di ricalcolare la varianza a ogni iterazione del CG (operazione costosa), il paper propone di calcolarla una sola volta per $d = p_0$. Sfruttando l'omogeneità della forma quadratica, si definisce
\[
