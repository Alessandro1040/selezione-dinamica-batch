...
\begin{mnbox}
  \textbf{Un ponte con i Metodi Numerici.}
  I metodi numerici classici studiano l'approssimazione di soluzioni di equazioni e problemi di ottimizzazione. In questo documento, estendiamo quegli stessi algoritmi (gradiente, Newton, gradiente coniugato) al contesto dei \emph{grandi dataset}, dove non possiamo permetterci di calcolare esattamente le quantità (gradiente, Hessiana) su tutti i dati. L'idea è quella di usarne \emph{approssimazioni stocastiche} e di controllare dinamicamente l'accuratezza di tali approssimazioni.
\end{mnbox}
...
La strategia naturale sarebbe: guarda tutti gli esempi, calcola la direzione di miglioramento (il gradiente), fai un passo in quella direzione, ripeti. Il problema? \textbf{Calcolare il gradiente su 100 milioni di esempi ad ogni passo \`e proibitivo.}

Le alternative classiche sono:
\begin{itemize}
    \item Usare \textbf{un solo esempio per volta} (metodo stocastico): veloce ma rumoroso, come navigare a vista nella nebbia.
    \item Usare \textbf{tutti gli esempi ad ogni passo} (metodo batch): preciso ma lentissimo.
\end{itemize}
...
\begin{definitionbox}
  ...
  \begin{mnbox}
    ...
    \end{itemize}
    Il \textbf{numero di condizionamento} $\kappa = L/\lambda$ gioca un ruolo cruciale: per una funzione quadratica $f(w) = \frac12 w^T A w$, con $A$ simmetrica definita positiva, $\kappa = \lambda_{\max}(A)/\lambda_{\min}(A)$. Nel metodo del gradiente con passo ottimo, l’errore decresce come
    \[
      f(w_k) - f(w_*) \leq \left(\frac{\kappa - 1}{\kappa + 1}\right)^{2k} (f(w_0) - f(w_*)).
    \]
    (RIVEDERE Quest'ultima frase e organizzare meglio anche la struttura: attualmente ci sta questa parte che parla del numero di condizionamento, poi si parla della norma di una matrice, e poi ancora del numero di condizionamento, quindi magari spostare tutto dopo la norma)
    Più $\kappa$ è grande (problema mal condizionato), più lenta è la convergenza. Nel Machine Learning, $\kappa$ può essere enorme, da qui la necessità di metodi più avanzati come Newton o gradiente coniugato.
  \end{mnbox}
\end{definitionbox}
