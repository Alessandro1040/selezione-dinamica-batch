...
g_i = \arg\min_{g \in [-\nu, \nu]} |d_i + g|,
\]
cioè si sceglie il subgradiente che rende il gradiente generalizzato \(d_i + g_i\) il più vicino possibile a zero. \textit{Con questa scelta vogliamo selezionare, tra tutti i subgradienti possibili rispetto al parametro $w_i$, quello che annulla $d_i + g_i$ oppure, quando questo non è possibile, quello che lo rende il più piccolo possibile in valore assoluto: in questo modo troviamo un minimo effettivo per i parametri, migliore dei minimi che otterremmo usando qualsiasi altro subgradiente.} Ne derivano i tre casi seguenti:
...
($\approx 4\times10^{-1}$): la curvatura varia pochissimo da una direzione
all'altra, perciò un passo scalare ben calibrato è adeguato in ogni direzione, mentre Newton-CG paga la soluzione
...
ai problemi in cui la curvatura è molto diversa da una direzione all'altra:
un passo fisso non può essere insieme abbastanza piccolo nella direzione ripida e abbastanza grande in quella piatta.
