...
Il meccanismo è il seguente: il dataset di $N$ esempi è diviso in training e
validation ($p$ è la percentuale in validation), il mini-batch è campionato
dal solo training set e la CCV continua ad aumentarne la dimensione, con
tetto $\lvert\mathcal{T}\rvert$ (la dimensione del training set) invece di
$N$.
...
L'applicazione espone sei iperparametri: la pazienza $P$ (numero di
valutazioni senza miglioramento prima di ricampionare, default 3), la
tolleranza relativa $\tau$ (default $10^{-4}$), la soglia assoluta
$\epsilon_{\mathrm{abs}}$ (default 0), la percentuale di validation $p$
(default 20\%), la frequenza di valutazione $f$ (default 1) e la strategia di
split (default fisso). Pazienza e tolleranza controllano la frequenza di
ricampionamento attraverso il criterio di miglioramento
$J_{\mathrm{val}} \le J_{\mathrm{val}}^{\mathrm{best}}(1-\tau)-\epsilon_{\mathrm{abs}}$:
una $\tau$ più grande alza la soglia richiesta per azzerare il contatore di
pazienza, quindi ricampiona più spesso; una $\tau$ più piccola la abbassa e
ricampiona meno spesso (nell'intervallo testato, $10^{-5}$--$10^{-3}$,
l'effetto risulta però ininfluente, come discusso qui sotto). La pazienza
$P$ agisce invece direttamente: valori piccoli ricampionano presto,
avvicinandosi alla \emph{base}; valori grandi tengono il batch più a lungo,
avvicinandosi a $M=\infty$. La percentuale $p$ bilancia la qualità della
stima di $J_{\mathrm{val}}$ con la quantità di dati di training; la
frequenza $f$ controlla la prontezza del criterio; la strategia dinamica
cambia il validation set a ogni ricampionamento.
