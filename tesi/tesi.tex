...
\begin{center}
  ...
  \textbf{[PLACEHOLDER] Inserire qui i ringraziamenti.}
\end{center}
\newpage
% ── ABSTRACT ────────────────────────────────────────────────────────────────
\begin{center}
...
Le variabili da tenere a zero sono individuate con il criterio del gradiente
generalizzato (Sezione~\ref{sec:l1}): per ogni coordinata $i$ si confronta la
derivata parziale della parte liscia, $\partial J(w)/\partial w_i$, con il costo
$\nu$. Se $w_i = 0$ e $-\nu \le \partial J(w)/\partial w_i \le +\nu$, il
subgradiente della norma in $0$ --- l'intervallo $[-\nu,+\nu]$ --- contiene la
componente liscia e $w_i = 0$ è un punto stazionario per quella coordinata: la
variabile resta ancorata a zero, contribuendo alla sparsità della soluzione. Se
invece $\partial J(w)/\partial w_i < -\nu$ (o $> +\nu$), la parte liscia
``vince'' sul costo $L_1$ e conviene liberare la variabile, assegnandole la
faccia positiva (o negativa) dell'ortante, così che la minimizzazione possa
spostarla da zero. È la strategia adottata dal metodo Newton-CG-$L_1$ descritto
nel Capitolo~\ref{sec:algoritmi}.
...
Passo calcolato & inverso della curvatura media lungo $s$ (ottimale) & media degli ultimi passi, obsoleta al passo successivo \\
...
\end{table}
\noindent\emph{Lettura.} Spiega perché il passo di Barzilai--Borwein è esatto e ottimale sul quadratico ma richiede una salvaguardia sui problemi generali (il denominatore $s^\top y$ può essere $\le0$ o $\approx0$): nella pratica la salvaguardia di BB-CCV è sufficiente a renderlo il metodo più veloce tra quelli testati.
...
media pesata delle curvature della Hessiana --- cioè un passo di Newton scalare
a costo nullo, poiché $s_k$ e $y_k$ sono quantità già disponibili. Per
robustezza sui problemi
