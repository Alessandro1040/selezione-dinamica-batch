...
Infine, per completare il confronto tra i metodi dell'applicazione,
riportiamo i test comparativi eseguiti con il pannello ``Analisi''
dell'applicazione interattiva (Sezione~\ref{sec:visualizzazione}) su Dynamic
GD, Newton-CG, Newton-CG~$L_1$ e BB-CCV (Appendice~\ref{app:bbccv}). Per ogni
iterazione $k$ il pannello riporta la norma dell'errore $\|w_k-w_*\|_2$. I
test sono stati eseguiti sulle tre funzioni obiettivo quadratiche
dell'applicazione: ben condizionata ($\kappa \approx 1.1$), molto mal
condizionata ($\kappa \approx 100$) e con termine incrociato. Le
Tabelle~\ref{tab:test_bencond}--\ref{tab:test_incrociato} riportano
$\|w_k-w_*\|_2$ a ogni iterazione, con i quattro algoritmi affiancati in
ordine: (a)~Dynamic GD, (b)~Newton-CG, (c)~Newton-CG~$L_1$, (d)~BB-CCV.

% Tabella 6.3 - quadratico ben condizionato ($\kappa \approx 1.1$)
\begin{table}[H]
\centering
\footnotesize
...
% Tabella 6.4 - quadratico molto mal condizionato ($\kappa \approx 100$)
\begin{table}[H]
\centering
\footnotesize
\renewcommand{\arraystretch}{0.85}
...
% Tabella 6.5 - quadratico con termine incrociato
\begin{table}[H]
\centering
\footnotesize
\renewcommand{\arraystretch}{0.85}
