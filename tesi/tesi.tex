...
\item Mal condizionato ($\kappa \gg 1$). Esiste una direzione quasi
  ...
  già così lentamente che il guadagno in coerenza prevale sul rischio
  di deriva. BB-CCV fa eccezione: il passo di Barzilai--Borwein lo porta alla
...
\end{tabular}
\end{table}
\noindent\emph{Lettura.} Errore a ogni iterazione di Dynamic GD su $\kappa\approx100$ (seed 42): a differenza dei problemi mal condizionati, il riuso con $M{=}\infty$ migliora l'errore finale ($3.71\times10^{-1}$ contro $4.67\times10^{-1}$ della base), perché la base converge già così lentamente che il guadagno in coerenza prevale sul rischio di deriva; su 5 seed migliora in 3 casi su 5 (Tabella~\ref{tab:riuso_robustezza}).
