...
\begin{center}
  ...
  \parbox{0.92\textwidth}{%
    ...
    famiglia per il supporto; in particolare, ringrazio:
  }
  ...
  Cristina Pesci, Emmanuel Nsia,\\
  Gaia Facioni, Gabriele Cirillo, Mario Prignano, Michele Aliffi,\\
  Luca Veneri, Matteo Pinato,\\
  Mariagiusi Nicodemo, Aurora Di Giovanna, Sofia De Angelis, Silvio Zylfi, Lorenzo Lausi,\\
  ...
  % SkyDxddy, Denace (rimossi dall'elenco; erano nella riga:
...
\begin{enumerate}
  ...
  condizionati.
  \item Miglioramento dei teoremi di convergenza. I risultati di convergenza
  dei metodi a campione dinamico sono raffinati rispetto al lavoro
  originale~\cite{byrd2012} e a Nocedal e Wright~\cite{nocedal2006}, e il
  contributo non è solo l'impiego della disuguaglianza di Polyak--Lojasiewicz
  nella forma forte $\|\nabla J(w)\|^2 \ge 2\lambda J(w)$: la ricorrenza di
  convergenza in aspettativa è anche risolta in forma chiusa, senza il fattore
  $1/2$ che l'argomento induttivo del lavoro originale introduce. In
  particolare, il fattore di contrazione del metodo del gradiente a campione
  dinamico passa da $1-\frac{(1-\theta)\lambda}{2L}$ a
  $1-\frac{(1-\theta)\lambda}{L}$, con il dimezzamento delle iterazioni
  necessarie per raggiungere l'accuratezza $\varepsilon$
  (Sezione~\ref{sec:convergenza-deterministica}); il tasso di convergenza in
  aspettativa diventa $\rho = \max\{1-\lambda/L,\, 1/a\}$ (da
  $\max\{1-\lambda/(4L),\, 1/a\}$), con costante $C$ esplicita; di conseguenza
  l'intervallo di ammissibilità per la crescita geometrica del batch è più
  ampio e il bound di complessità totale ha una costante migliore
  (Corollario~5.2).
...
duplice: fornisce una giustificazione statistica alla regola di aggiornamento
e si adatta automaticamente alla difficoltà locale del problema, senza
richiedere la stima di $\kappa$.
...
convergenza. I metodi di Newton incorporano la curvatura dell'Hessiana e
raggiungono un tasso di convergenza locale quadratico (o superlineare), al
costo di risolvere a ogni iterazione un sistema lineare
$H_k d = -g_k$; si veda il testo di Nocedal e Wright~\cite{nocedal2006} per
una trattazione completa.

Su larga scala la costruzione esplicita dell'Hessiana è impraticabile e si
usano quindi sue approssimazioni. I metodi
...
campionaria dei prodotti Hessiana-vettore, così da non sprecare iterazioni
...
\nu\|w\|_1 = \nu \sum_{i=1}^{m} |w_i|,
\]}
dove $\nu > 0$ ne controlla l'intensità. L'effetto è
duplice: i coefficienti grandi vengono penalizzati, scoraggiando
l'adattamento eccessivo ai dati, e la soluzione risulta \emph{sparsa}, cioè con
molti coefficienti esattamente nulli. L'intuizione è la seguente: a differenza
della norma $L_2$, il cui costo marginale tende a zero per coefficienti piccoli,
la norma $L_1$ ``costa'' sempre $\nu$ per ogni unità di coefficiente, anche
vicino a zero. Conviene quindi azzerare le variabili il cui contributo alla
perdita non supera tale costo: per questo la $L_1$ è uno strumento fondamentale
per l'interpretabilità dei modelli ad alta dimensionalità (il principio alla
base del LASSO).

La non differenziabilità della norma $L_1$ nei punti in cui un coefficiente si
annulla richiede di adattare i metodi classici del primo ordine. In questa
...
solo su quelle libere, dove la $L_1$ è differenziabile, mentre le altre vengono
mantenute a zero tramite una ricerca lineare proiettata.
...
$w_i = 0$ viene riconosciuto come un vero minimo ogni volta che lo è e
l'algoritmo non rischia di allontanarsene: è la strategia adottata dal metodo
Newton-CG-$L_1$ del Capitolo~\ref{sec:algoritmi}.
...
\begin{tikzpicture}[
  ...
  \node[box, right=2.1cm of ccv, text width=4.6cm, minimum height=1.6cm] (grow) {
    CCV violata:\\[2pt]
    aumento del batch\\[1pt]
    (vale per la prossima estrazione)\\[2pt]
  ...
  \draw[arrow] (grad) -- (ccv);
  \draw[arrow] (ccv) -- (ls) node[label, midway, right] {in ogni caso};
  \draw[arrow] (ls) -- (upd);
  \draw[arrow] (upd) -- (conv);
  ...
  \draw[arrow] (grow.east) -- ++(0.8,0) |- ($(batch.east)+(0.8,0)$) -- (batch.east)
    node[label, pos=0.25, right] {nuovo $n_k$ (prossima estrazione)};
...
$\to$ aggiornamento. La verifica della CCV non interrompe l'iterazione corrente,
che si completa con il campione gi\`a estratto: se la CCV \`e violata la
dimensione del batch viene aumentata e il nuovo valore \`e usato dalla
estrazione del campione successivo.}
...
\begin{actionbox}
  Gradiente a Campione Dinamico\\[0.1cm]
  \textit{Input:} $w_0$ (punto iniziale), $\mathcal{S}_0$ (batch iniziale), $\theta \in (0,1)$ (tolleranza).\\[0.05cm]
  \textit{Ripeti per ogni } $k \in \mathbb{N}$ \textit{ fino a convergenza:}
  \begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\topsep}{3pt}\setlength{\parskip}{0pt}
...
\begin{actionbox}
  ...
  \setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}\setlength{\parskip}{0pt}
    ...
    \setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}\setlength{\parskip}{0pt}
      ...
      \item Se $\dfrac{\|\widehat{\mathcal{V}}\|_1}{|\mathcal{S}_{k+1}|} > \theta^2 \|\nabla J_{\mathcal{S}_{k+1}}(w_{k+1})\|_2^2$, aumenta il batch per la prossima estrazione:
      \[
...
\begin{tikzpicture}[
  ...
  % Nodo laterale (aumento batch)
  \node[box, right=2.1cm of ccv, text width=4.6cm, minimum height=1.6cm] (grow) {
    CCV violata: aumento del batch\\[1pt]
    (vale per la prossima estrazione;\\[1pt]
    anche $\mathcal{H}_{k+1}$ è riscalato)\\[2pt]
    $|\mathcal{S}_{k+1}| \leftarrow \left\lceil \dfrac{\|\widehat{\mathcal{V}}\|_1}{\theta^2 \|g_{k+1}\|^2} \right\rceil$
  };
  ...
  \draw[arrow] (ccv) -- (conv) node[label, midway, right] {in ogni caso};
  \draw[arrow] (conv) -- (end) node[label, midway, above] {sì};
  ...
  \draw[arrow] (grow.east) -- ++(0.8,0) |- ($(recamp.east)+(0.8,0)$) -- (recamp.east)
    node[label, pos=0.25, right] {nuovo $n_{k+1}$ (prossima estrazione)};
...
ricampionamento, la CCV decide la dimensione del campione per l'iterazione
successiva, mentre l'iterazione corrente si completa comunque con il campione
gi\`a estratto.}
\label{fig:newton_cg}
\end{figure}
...
è esattamente $z_k$. Per le variabili libere ($z_k^i \neq 0$) il subgradiente
scelto è $g_k^i = \nu\,z_k^i$, mentre per le variabili dell'active set
($z_k^i = 0$) è $g_k^i = -\partial J_{\mathcal{S}_k}(w_k)/\partial w_i$,
cosicché la corrispondente componente del gradiente generalizzato è nulla.
In forma compatta si scrive quindi
\[
\widetilde{\nabla}F_{\mathcal{S}_k}(w_k) = \nabla J_{\mathcal{S}_k}(w_k) + g_k,
\]
con $g_k^i = \nu\,z_k^i$ quando $z_k^i \neq 0$: la scrittura
$\nabla J_{\mathcal{S}_k}(w_k) + \nu z_k$ vale solo sulle coordinate libere,
non su quelle dell'active set. L'algoritmo ha scelto quella posizione basandosi sulla condizione di stazionarietà:
\begin{itemize}
...
\begin{actionbox}
  ...
  $\mathcal{S}_0$ (batch iniziale per il gradiente) e
  $\mathcal{H}_0 \subset \mathcal{S}_0$ con $|\mathcal{H}_0| < |\mathcal{S}_0|$,
  $R = |\mathcal{H}_0|/|\mathcal{S}_0|$ (rapporto costante),
  $\theta \in (0,1)$ (tolleranza per la varianza del gradiente),
  ...
  \begin{enumerate}
    \item
    Calcola $g_k = \nabla J_{\mathcal{S}_k}(w_k)$ e la varianza campionaria
    $\widehat{\mathcal{V}} = \Var_{i \in \mathcal{S}_k}(\nabla \ell(w_k;i))$.
    ...
    \begin{enumerate}
      \item Calcola $\widetilde{\nabla}F_{\mathcal{S}_k}(w_k) = \nabla J_{\mathcal{S}_k}(w_k) + g_k$, dove $g_k$ è il subgradiente scelto dalla regola di proiezione \eqref{eq:5_43}; sulle coordinate libere $g_k^i = \nu z_k^i$.
    ...
    \item Verifica della CCV e ricampionamento.
    Se $\dfrac{\|\widehat{\mathcal{V}}\|_1}{n_k} > \theta^2 \|g_k\|_2^2$, aumenta il batch:
    \[
    n_k \leftarrow \left\lceil \frac{\|\widehat{\mathcal{V}}\|_1}{\theta^2 \|g_k\|_2^2} \right\rceil ,
    \]
    dove $\widehat{\mathcal{V}}$ e $g_k$ sono quelli del campione corrente, e la
    nuova dimensione è usata per $\mathcal{S}_{k+1}$. Scegli quindi
    $\mathcal{S}_{k+1}$ di dimensione $n_k$ e
    $\mathcal{H}_{k+1} \subseteq \mathcal{S}_{k+1}$ con
    $|\mathcal{H}_{k+1}| = R\,|\mathcal{S}_{k+1}|$.
  \end{enumerate}
\end{actionbox}

\begin{notebox}
  Campionamento dinamico nel caso $L_1$. L'Algoritmo~6.1 di~\cite{byrd2012}
  mantiene fisse le dimensioni dei campioni ($|\mathcal{S}_{k+1}|=|\mathcal{S}_0|$)
  e, nell'esperimento, usa $|\mathcal{S}_k|=N$ sottocampionando soltanto
  l'Hessiana. L'implementazione qui presentata estende il metodo aggiornando
  $n_k$ con la stessa condizione di controllo della varianza usata per il
  gradiente a campione dinamico, applicata al gradiente liscio: la parte non
  liscia di $F$ è deterministica in $w$, quindi l'unica sorgente di rumore è
  $\nabla J_{\mathcal{S}_k}(w_k)$. La soglia $\theta^2\|g_k\|^2$ (anziché
  $\theta^2\|\widetilde{\nabla}F\|^2$) evita che la condizione degeneri in
  prossimità della soluzione sparsa, dove $\widetilde{\nabla}F$ è nullo sulle
  coordinate dell'active set mentre $\nabla J$ resta non nullo su quelle attive;
  la variante con soglia $\theta^2\|\widetilde{\nabla}F\|^2$ è lasciata a lavoro
  futuro.
\end{notebox}
...
\begin{tikzpicture}[
  ...
  % Nodi principali (colonna centrale)
  \node[io] (init) {Inizializzazione\\[2pt] $w_0=0$, $\mathcal{S}_0$, $\mathcal{H}_0$, $R$, $\theta$, $\nu$, $\eta$, $\sigma$};
  \node[box, below=of init, text width=4.6cm, minimum height=1.4cm] (subgrad) {Calcola $\widetilde{\nabla}F_{\mathcal{S}_k}(w_k)$\\[1pt] $=\nabla J_{\mathcal{S}_k}(w_k)+g_k$\\[1pt] e determina $z_k$, $\mathcal{A}_k$};
  ...
  \node[box, below=of ls, text width=5.6cm, minimum height=2.2cm] (recamp) {Verifica CCV e ricampionamento\\[2pt] se $\dfrac{\|\widehat{\mathcal{V}}\|_1}{n_k} > \theta^2 \|g_k\|^2$: \ $n_k \leftarrow \left\lceil \dfrac{\|\widehat{\mathcal{V}}\|_1}{\theta^2 \|g_k\|^2} \right\rceil$\\[2pt] $\mathcal{S}_{k+1}$ con $|\mathcal{S}_{k+1}| = n_k$; \ $\mathcal{H}_{k+1} \subseteq \mathcal{S}_{k+1}$ con $|\mathcal{H}_{k+1}| = R\,|\mathcal{S}_{k+1}|$};
...
proiettata mantiene le variabili dell'active set a zero; la dimensione del
campione è aggiornata dalla stessa condizione di controllo della varianza usata
dagli altri metodi (applicata al gradiente liscio).}
\label{fig:newton_l1}
\end{figure}
...
adattivo di Barzilai--Borwein~\cite{barzilai1988}, che stima la curvatura di
$J$ lungo la direzione percorsa usando soltanto le quantità già disponibili,
senza richiedere la Hessiana.
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (batch) -- (grad);
  \draw[arrow] (grad) -- (ccv);
  \draw[arrow] (ccv) -- (bb) node[label, midway, right] {in ogni caso};
  \draw[arrow] (bb) -- (ls);
  \draw[arrow] (ls) -- (upd);
  \draw[arrow] (upd) -- (conv);
...
con eventuale aumento di $n_k$ per il campione successivo (l'iterazione corrente
si completa con il campione gi\`a estratto). La differenza è nel passo: per $k > 0$ si stima
...
\begin{itemize}
  ...
  e $1.9$, da cui $\kappa = 2.1/1.9 \approx 1.1$. La perdita individuale è
  $\ell(w;i) = (w_1 - a_i)^2 + (w_2 - b_i)^2 + c_i\,w_1 w_2$, dove $a_i$ e $b_i$
  sono variabili casuali indipendenti di media $1$ e $-2$ e deviazione standard
  $\sigma = 0.2$, mentre $c_i$ ha media $0.1$ e deviazione standard $0.05$; i
  valori sono poi ricentrati sulla media campionaria (dataset ``centrato'',
  ...
  Sezione~\ref{app:riuso}. La perdita individuale è
  $\ell(w;i) = 20\,(w_1 - a_i)^2 + (w_2 - b_i)^2$, dove $a_i$ e $b_i$ sono
  variabili casuali indipendenti di media $1$ e $-2$ e deviazione standard
  $\sigma = 0.2$; i valori sono poi ricentrati sulla media campionaria (dataset
  ``centrato'', Sezione~\ref{sec:architettura}).
  \item Quadratica molto mal condizionata ($\kappa \approx 100$):
  $J(w) = 100\,(w_1 - 1)^2 + (w_2 + 2)^2$ (preset ``Quadratica molto mal
  condizionata ($\kappa \approx 100$)''). La perdita individuale è
  $\ell(w;i) = 100\,(w_1 - a_i)^2 + (w_2 - b_i)^2$, dove $a_i$ e $b_i$ sono
  variabili casuali indipendenti di media $1$ e $-2$ e deviazione standard
  $\sigma = 0.2$; i valori sono poi ricentrati sulla media campionaria (dataset
  ``centrato'', Sezione~\ref{sec:architettura}).
  ...
  $1.5$, quindi $\kappa = 2.5/1.5 \approx 1.67$. La perdita individuale è
  $\ell(w;i) = (w_1 - a_i)^2 + (w_2 - b_i)^2 + c_i\,(w_1 - a_i)(w_2 - b_i)$,
  dove $a_i$ e $b_i$ sono variabili casuali indipendenti di media $1$ e $-2$ e
  deviazione standard $\sigma = 0.2$, mentre $c_i$ ha media $0.5$ e deviazione
  standard $0.05$; i valori sono poi ricentrati sulla media campionaria (dataset
  ``centrato'', Sezione~\ref{sec:architettura}).
...
originale~\cite{byrd2012}: l'impiego della CCV anche nel metodo $L_1$ e il dataset
sintetico centrato descritto sopra. Nel caso $L_1$ la CCV è applicata al
gradiente liscio $\nabla J_{\mathcal{S}_k}(w_k)$: la parte non liscia
dell'obiettivo è deterministica in $w$, quindi l'unica sorgente di rumore è
$\nabla J_{\mathcal{S}_k}(w_k)$; la soglia $\theta^2\|g_k\|^2$ evita inoltre che
la condizione degeneri in prossimità della soluzione sparsa, dove
$\widetilde{\nabla}F$ è nullo sulle coordinate dell'active set mentre $\nabla J$
resta non nullo su quelle attive. La variante con soglia
$\theta^2\|\widetilde{\nabla}F\|^2$ è lasciata a lavoro futuro. Per il resto, l'implementazione è fedele
...
Presentiamo i risultati numerici del setup descritto nella
Sezione~\ref{sec:setup}, eseguiti sui problemi test dell'applicazione web
(Sezione~\ref{sec:visualizzazione}). Per ogni problema riportiamo l'errore
finale $\|w_{30}-w_*\|_2$ al termine delle 30 iterazioni (seed 42) e
discutiamo il comportamento qualitativo dei quattro algoritmi Dynamic GD,
Newton-CG, Newton-CG~$L_1$ e BB-CCV (Sezione~\ref{sec:bbccv}); i valori sono quelli del pannello ``Analisi''
dell'applicazione. Le stesse versioni \emph{base} (ricampionamento a ogni
iterazione) sono riprese nell'esperimento di riuso della
Sezione~\ref{app:riuso}, che ne riporta l'errore finale $e_{30}$ per tutti i
problemi nella Tabella~\ref{tab:riuso_sintesi}.
...
$1.0991\times10^{-14}$, raggiunta da BB-CCV sul problema molto mal condizionato
$\kappa\approx100$; Sezione~\ref{app:riuso},
Tabella~\ref{tab:riuso_sintesi}) sono tutti colorati di verde chiaro.

\paragraph{Osservazioni comparative.} Dai risultati sui problemi quadratici
emergono tre indicazioni; il quinto problema, la Funzione di Rosenbrock, è
discusso qui sotto.
...
Chiude il confronto il problema non quadratico ``Funzione di Rosenbrock''
della Sezione~\ref{sec:setup}, eseguito con lo stesso setup dei problemi
quadratici (seed 42, $w_0=(2,-3)$, $\alpha=0.1$, $\theta=0.5$, $n_0=5$, 30
iterazioni); i valori sono generati con lo script di riproduzione dei
risultati del Capitolo (NumPy di sistema).
...
Per i metodi di Newton l'Hessiana è stimata sul sottoinsieme $H_k \subseteq
...
coerente con la Sezione~\ref{sec:setup}); $H_k$ è ricampionata insieme a
$S_k$ (modalità \emph{Legato a $S_k$}, default). Nella modalità
\emph{Indipendente da $S_k$} con $M_H=\infty$ l'Hessiana resta invece in uso
finché la CCV è soddisfatta, senza seguire il contatore del batch; con
$M=\infty$ le due modalità coincidono (Sezione~\ref{app:riuso-descrizione}) e
la modalità indipendente è quindi testata solo con batch finito.

\subsubsection{Risultati numerici}\label{app:riuso-risultati}

Il quadro emerso è coerente con l'analisi della
Sezione~\ref{app:riuso-meccanismo} e si riassume nei punti seguenti.
\begin{itemize}
...
superiore alla media della \emph{base} ($2.09\times10^{-1}$): sulla Funzione di Rosenbrock il
criterio peggiora su tutti i metodi ($3.84\times10^{-1}$ contro
...
al contrario, il criterio è controproducente rispetto alla
...
\begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\topsep}{3pt}\setlength{\parskip}{0pt}
  ...
  \item Confronto sistematico con GD, SGD, SVRG e SAGA. Gli esperimenti del
  Capitolo~\ref{sec:esperimenti} confrontano i quattro metodi a campionamento
  dinamico tra loro e, sul piano teorico, con i bound di complessità di SGD, ma
  non includono un confronto empirico diretto con i metodi stocastici classici
  del Capitolo~\ref{sec:lavori}, in particolare GD, SGD e i metodi a varianza
  ridotta SVRG~\cite{johnson2013} e SAGA~\cite{defazio2014}. Una direzione
  naturale è implementarli nella stessa piattaforma (Pyodide/NumPy)
  dell'applicazione web di questa tesi, per misurarli nelle identiche condizioni
  sperimentali e confrontarli con il campionamento dinamico in termini di costo
  computazionale totale, numero di valutazioni di gradienti individuali
  $\nabla\ell(w;i)$ oppure tempo di esecuzione, anziché a parità di iterazioni,
  che non riflette il lavoro effettivo e favorirebbe i metodi dal costo per
  iterazione più basso.
\end{enumerate}
...
che il vettore dei parametri abbia dimensione $m$: $w \in \mathbb{R}^m$.
...
machine learning,''
\textit{SIAM Journal on Optimization}, vol.~21, no.~3, pp.~977--995, 2011.
...
\textit{IMA Journal of Numerical Analysis}, vol.~8, no.~1, pp.~141--148,
1988.
