...
\subsection{Validazione su un benchmark musicale: riconoscimento della nota con NSynth}\label{sec:nsynth}
...
Il modello è una regressione
logistica multinomiale: per ogni clip $x$ (vettore di 24 features) assegna a
ogni nota $c$ un punteggio
\[
  s_c(x) = w_{c,1}x_1 + \cdots + w_{c,24}x_{24} + b_c,
\]
cioè una combinazione lineare delle features (ogni componente cromatica
contribuisce in modo indipendente, con il proprio peso), e trasforma i dodici
punteggi in probabilità con la softmax. Il modello è ``lineare'' nel senso che
la decisione è una somma pesata degli input, senza interazioni tra le features
(in due dimensioni la separazione tra due classi sarebbe una retta); l'unica
...
completo con la sua evoluzione temporale, ma 24 statistiche aggregate (media e
deviazione standard delle 12 componenti cromatiche nel tempo); (ii)~la
decisione è lineare e non usa features apprese.
...
di ciascuna, con la \emph{firma} (ingressi $\to$ uscite) e le formule esatte
...
\node[box, text width=11.0cm, minimum height=1.5cm] (algs) at (0.0, -3.0) {Quattro algoritmi\\[2pt] (listati B.1--B.4), logica invariata\\[1pt] Dynamic GD, Newton-CG, Newton-CG $L_1$, BB-CCV};
...
funzioni del nucleo forniscono agli algoritmi dei listati B.1--B.4 proprio gli oggetti
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small},
  io/.style={draw=gray!60, fill=gray!8, rounded corners=3pt, text width=6.6cm, align=center, minimum height=1.0cm, inner sep=5pt},
  box/.style={draw=coverblue, fill=coverblue!6, rounded corners=3pt, text width=6.6cm, align=center, minimum height=1.0cm, inner sep=5pt},
  arrow/.style={-{Stealth[length=2.5mm]}, thick, color=coverblue},
  label/.style={font=\scriptsize, fill=white, inner sep=1.5pt}
]

  % Firma
  \node[io, text width=6.4cm] (sig) at (0, 5.5) {Firma: \texttt{estrai\_chroma}(clip)\\[2pt] $\to x_i \in \mathbb{R}^{24}$};

  % Passi
  \node[box] (load) at (0, 3.0) {Caricamento\\ $y = \texttt{librosa.load}(clip,\; \texttt{sr}{=}16\,000,\; \texttt{mono})$\\[1pt] $y \in \mathbb{R}^{T}$, $T \approx 64\,000$ campioni};
  \node[box] (chroma) at (0, 0.0) {Chromagramma\\ $C = \texttt{chroma\allowbreak\_stft}(y)$\\[1pt] energia nelle 12 classi per frame,\\[1pt] $C \in \mathbb{R}^{12 \times F}$};
  \node[box] (stat) at (0, -3.0) {Statistiche nel tempo\\ $x_i = \bigl[\, \mathrm{mean}(C);\; \mathrm{std}(C) \bigr]$\\[1pt] 12 medie + 12 deviazioni standard};

  % Uscita
  \node[io] (out) at (0, -5.5) {Uscita: $x_i \in \mathbb{R}^{24}$\\[2pt] (riga di $X$ per \texttt{costruisci\_dati})};
...
\begin{tikzpicture}[
  node distance=1.0cm and 1.4cm,
  every node/.style={font=\small},
  io/.style={draw=gray!60, fill=gray!8, rounded corners=3pt, text width=7.2cm, align=center, minimum height=1.0cm, inner sep=5pt},
  box/.style={draw=coverblue, fill=coverblue!6, rounded corners=3pt, text width=7.2cm, align=center, minimum height=1.0cm, inner sep=5pt},
  arrow/.style={-{Stealth[length=2.5mm]}, thick, color=coverblue},
  label/.style={font=\scriptsize, fill=white, inner sep=1.5pt}
]

  % Firma
  \node[io, text width=6.8cm] (sig) at (0, 6.0) {Firma: \texttt{costruisci\_dati}(split)\\[2pt] $\to (X,\; y)$};

  % Passi
  \node[box] (loop) at (0, 3.5) {Per ogni clip dello split\\ $x_i = \texttt{estrai\_chroma}(clip_i)$,\\[1pt] $y_i = \texttt{pitch}_i \bmod 12$\\[1pt] ($0{=}C, \dots, 11{=}B$)};
  \node[box] (mat) at (0, 0.8) {Matrice e etichette\\ $X \in \mathbb{R}^{N \times 24}$, \quad $y \in \{0,\dots,11\}^{N}$};
  \node[box] (std) at (0, -1.9) {Standardizzazione (sul training set)\\ $\mu = X_{tr}.\mathrm{mean}(0)$, \quad $\sigma = X_{tr}.\mathrm{std}(0)$\\[1pt] $\sigma[\sigma < 10^{-8}] = 1$; \quad $X \leftarrow (X - \mu)/\sigma$};

  % Uscita
  \node[io] (out) at (0, -4.8) {Uscita: $X$ (normalizzata), $y$\\[2pt] train: split valid (12\,678 clip),\\[1pt] test (4\,096 clip)};
...
I listati B.1--B.4 operano per-esempio: a ogni iterazione richiamano in loop
...
\begin{tikzpicture}[
  ...
  % Firma
  \node[io, text width=6.6cm] (sig) at (0, 5.5) {Firma: \texttt{loss\_i}($w$, $i$)\\[2pt] $\to \ell_i(w)$, per il singolo esempio $i$};
...
\begin{tikzpicture}[
  ...
  % Firma
  \node[io, text width=6.4cm] (sig) at (0, 5.5) {Firma: \texttt{grad\_i}($w$, $i$)\\[2pt] $\to \nabla \ell_i(w) \in \mathbb{R}^{300}$};

  % Passi
  \node[box] (fwd) at (0, 3.0) {Forward\\ $z = W x$ (shift del massimo), \quad $p = \mathrm{softmax}(z)$\\[1pt] $e_{y_i}$: one-hot della classe vera};
  \node[box] (err) at (0, 0.2) {Errore di output\\ $G = (p - e_{y_i})\, x^{\top}$ \quad (prodotto esterno $12 \times 25$)};
  \node[box] (reg) at (0, -2.6) {Regolarizzazione\\ $G[:, :-1] \leftarrow G[:, :-1] + \lambda\, W_{\mathrm{nb}}$};

  % Uscita
  \node[io] (out) at (0, -5.5) {Uscita: $\nabla \ell_i(w) = \mathrm{vec}(G)$};
...
\begin{tikzpicture}[
  ...
  % Firma
  \node[io, text width=6.6cm] (sig) at (0, 5.5) {Firma: \texttt{hessvec\_i}($w$, $i$, $v$)\\[2pt] $\to \nabla^2 \ell_i(w)\, v$};

  % Passi
  \node[box] (W) at (0, 3.0) {Pesi e direzione\\ $V = \mathrm{reshape}(v, 12, 25)$, \quad $q = V x$};
  \node[box] (r) at (0, 0.2) {Derivata della softmax\\ $r = p \odot q - p\,(p \cdot q)$};
  \node[box] (hv) at (0, -2.6) {Prodotto esterno\\ $Hv = r\, x^{\top}$, \quad $Hv[:, :-1] \leftarrow Hv[:, :-1] + \lambda\, V[:, :-1]$};

  % Uscita
  \node[io] (out) at (0, -5.5) {Uscita: $\mathrm{vec}(Hv)$\\[2pt] per il gradiente coniugato (B.2 e B.3)};
...
(Figura~\ref{fig:nota_grad_full}); serve per le metriche di valutazione
(norma del gradiente finale) e per il subgradiente del metodo $L_1$.
...
\begin{tikzpicture}[
  ...
  % Firma
  \node[io, text width=6.8cm] (sig) at (0, 6.0) {Firma: \texttt{grad\_full}($w$)\\[2pt] $\to \nabla J(w)$ (su tutto il training set, vettorizzato)};

  % Passi
  \node[box] (Z) at (0, 3.5) {Logits su tutto il train\\ $Z = X_{\mathrm{aug}} W^{\top} \in \mathbb{R}^{N\times 12}$, shift del massimo,\\[1pt] $P = \mathrm{softmax}(Z)$ (per righe)};
  \node[box] (Y) at (0, 1.0) {One-hot\\ $Y$: matrice $N\times12$ con $Y_{i,y_i} = 1$};
  \node[box] (G) at (0, -1.5) {Media del gradiente\\ $G = (P - Y)^{\top} X_{\mathrm{aug}}\, /\, N$};
  \node[box] (reg) at (0, -4.0) {Regolarizzazione\\ $G[:, :-1] \leftarrow G[:, :-1] + \lambda\, W[:, :-1]$};

  % Uscita
  \node[io] (out) at (0, -6.5) {Uscita: $\nabla J(w) = \mathrm{vec}(G)$\\[2pt] norma finale del gradiente (e subgradiente $L_1$)};
...
(Figura~\ref{fig:nota_batch}). In sintesi, gli stessi algoritmi validati sui
