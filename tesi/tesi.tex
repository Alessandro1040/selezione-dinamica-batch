...
\end{center}
\noindent
Ringrazio tutti i miei insegnanti, che con la loro passione mi hanno trasmesso
il valore dello studio e della curiosità, e la SMIA, il corso di laurea in
Scienze Matematiche per l'Intelligenza Artificiale, che mi ha dato
l'opportunità di trasformare questa passione in un percorso di studi. Un
ringraziamento particolare va a Giovanni Adelfio, Alessandro Pisarra, Emmanuel
Nsia, Michele Aliffi e Lorenzo Salis, che mi hanno accompagnato e motivato
lungo il cammino. Ringrazio infine la mia famiglia, per il sostegno costante e
incondizionato, e chiunque, in qualsiasi modo, mi abbia spinto a fare di più.
\newpage
% ── ABSTRACT ────────────────────────────────────────────────────────────────
\begin{center}
...
\begin{enumerate}
  \item Rilassamento dell'ipotesi di convessità forte. Un primo
  obiettivo è estendere l'analisi alla condizione di Polyak--Lojasiewicz~\cite{polyak1964, karimi2016}
...
La funzione \texttt{dynamic\_gd} riceve il punto iniziale $w_0$, la tolleranza
$\theta$ per la CCV, il numero massimo di iterazioni, il passo iniziale
\texttt{alpha} per la line search e la dimensione iniziale del batch
\texttt{batch0}. Converte $w_0$ in un array NumPy, forza la dimensione del
batch ad almeno 2 (così la varianza campionaria è definita) e inizializza le
liste \texttt{history} e \texttt{batch\_sizes}, che registreranno la
traiettoria dei pesi e le dimensioni del batch a ogni iterazione.

\begin{pythoncode}[title= Dynamic GD]

# Ciclo sulle iterazioni: estrazione del batch e calcolo del gradiente
...
A ogni iterazione si estrae senza reinserimento un campione $\mathcal{S}_k$ di
$n_k = n$ indici da $\{1,\dots,N\}$ e si calcola il gradiente del batch
$g_k = \frac{1}{n_k}\sum_{i \in \mathcal{S}_k} \nabla \ell(w_k; i)$.
...
Si stima la varianza campionaria dei gradienti del batch e si verifica la CCV
$\|\widehat{\mathcal{V}}\|_1 / n_k \le \theta^2 \|g_k\|_2^2$: se è violata,
la dimensione del batch viene aumentata secondo la regola di aggiornamento
della Sezione~\ref{sec:algoritmi}.
...
La direzione di discesa è l'antigradiente $d_k = -g_k$, e una line search con
backtracking cerca il passo $\alpha_k$ che soddisfa le condizioni di Wolfe
(diminuzione sufficiente e curvatura). Dopo al più 30 tentativi, se nessun
passo è accettabile, si pone $\alpha_k = 0$ e si aggiorna
$w_{k+1} = w_k + \alpha_k d_k$.

\begin{pythoncode}[title= Dynamic GD]

# Criterio di arresto e restituzione dei risultati
...
Infine si controlla la norma del gradiente esatto $\|\nabla J(w_k)\|_2$,
calcolato su tutto il dataset tramite \texttt{grad\_full}: se è minore di
$10^{-6}$ si considera raggiunta la convergenza. Si registrano il punto
corrente e la dimensione del batch, e la funzione restituisce le liste
\texttt{history} e \texttt{batch\_sizes}. L'ultima riga mostra la chiamata
...
# Gradiente coniugato: risolve A x = b con A operatore Hessiana-vettore
import numpy as np
...
La funzione \texttt{cg} risolve il sistema $Ax = b$ con il metodo del
gradiente coniugato, dove $A$ è l'operatore Hessiana-vettore (non la matrice
esplicita). Parte da $x_0 = 0$, residuo $r_0 = b$ e direzione $p_0 = r_0$; a
ogni iterazione calcola la curvatura $\text{pHp} = p^T A p$ e il test di
arresto adattivo $\|r\|^2 \le \gamma\|x\|^2 + 10^{-16}$, dove $\gamma$
bilanci l'errore di troncamento del CG con il rumore statistico del
...
La funzione \texttt{newton\_cg} riceve il punto iniziale $w_0$, la tolleranza
$\theta$, il numero massimo di iterazioni, il passo iniziale \texttt{alpha},
la dimensione iniziale del batch \texttt{batch0}, il rapporto $R$ tra la
dimensione del campione per l'Hessiana e quello per il gradiente, e il numero
massimo di iterazioni del CG \texttt{maxcg}.
...
A ogni iterazione si estrae il campione $\mathcal{S}_k$ e si calcola il
gradiente approssimato $g_k = \frac{1}{n_k}\sum_{i \in \mathcal{S}_k}
\nabla \ell(w_k; i)$.
...
La dimensione del campione per l'Hessiana è $n_h = R\,n_k$: si estraggono $n_h$
indici dal batch corrente e si definisce l'operatore \texttt{Hv} che calcola
il prodotto $\nabla^2 J_{\mathcal{H}_k}(w_k)\,v$ senza costruire la matrice.
...
Il coefficiente $\gamma$ stima la varianza dei prodotti Hessiana-vettore lungo
la direzione iniziale $p_0 = -g_k$; con questa soglia il CG restituisce la
direzione di Newton approssimata $d_k$.
...
La line search con backtracking cerca il passo $\alpha_k$ che soddisfa le
condizioni di Wolfe. Se la direzione di Newton non è di discesa
($g_k^T d_k \ge 0$), si ripiega sull'antigradiente.
...
Dopo l'aggiornamento si verifica la CCV su un nuovo campione: se è violata, la
dimensione del batch cresce secondo la regola della Sezione~5. Se la norma del
gradiente esatto scende sotto $10^{-6}$ si interrompe il ciclo; altrimenti si
registrano pesi e dimensioni del batch e si procede con l'iterazione
successiva.
...
Il listato implementa il metodo Newton-CG per il problema regolarizzato
$F(w) = J(w) + \nu\|w\|_1$ descritto nella Sezione~\ref{sec:algoritmi}.

\begin{pythoncode}[title= Newton-L1]

# Import, firma della funzione e ausiliarie
import numpy as np
...
Le tre funzioni ausiliarie valutano l'obiettivo regolarizzato sul batch
$F_{\mathcal{S}_k}(w) = J_{\mathcal{S}_k}(w) + \nu\|w\|_1$, calcolano il
gradiente generalizzato del problema $L_1$ (Sezione~5.3) e proiettano un
vettore sulla faccia ortante $\Omega_k$ caratterizzata da $z$.

\begin{pythoncode}[title= Newton-L1]

# Ciclo principale: batch, gradiente e vettore z_k
    for k in range(max_iter):
        indices_S = np.random.choice(N, size=n, replace=False)
...
Il vettore $z_k$ codifica il segno atteso di ciascuna componente all'ottimo e
definisce la faccia ortante attiva; il gradiente generalizzato e la sua norma
permettono di verificare la stazionarietà.
...
Se la norma del subgradiente è trascurabile il punto è stazionario e il ciclo
si interrompe.
...
Sulle variabili libere (quelle con $z_i \neq 0$) si risolve il sistema di
Newton con il gradiente coniugato Hessian free, usando solo prodotti
$\nabla^2 F\, v$ e la tolleranza $\eta\,\|g_{\text{free}}\|$.
...
La ricerca lineare proiettata applica la condizione di Armijo all'obiettivo
regolarizzato, proiettando i passi sulla faccia ortante $\Omega_k$.
...
Anche per il caso $L_1$ la CCV è applicata al gradiente della parte liscia sul
nuovo campione: se è violata il batch cresce; la convergenza è verificata
sulla norma del gradiente esatto di $J$.
