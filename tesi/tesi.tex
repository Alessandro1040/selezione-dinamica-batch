...
\begin{tikzpicture}[
  ...
  % Flusso unico: dati -> feature -> rete -> algoritmi -> valutazione
  \node[io] (data) {Dati: clip audio\\[2pt] (addestramento e test)};
  \node[box, right=of data] (feat) {Rappresentazione e normalizzazione\\[2pt] ogni clip $\to$ vettore $x_i \in \mathbb{R}^d$};
  \node[box, right=of feat] (net) {Definizione della rete\\[2pt] parametri $w = \mathrm{vec}(W_l, b_l)$};
  \node[box, below=of net] (algs) {Esecuzione dei quattro algoritmi\\[2pt] (listati B.1--B.4)};
  \node[io, right=of algs] (eval) {Valutazione su test\\[2pt] accuratezza, figure, tabelle};
...
\begin{tikzpicture}[
  ...
  % Colonna sinistra: batch, forward, errore
  ...
  % Colonna destra: retropropagazione e uscita
  \node[box, right=2.0cm of err] (bp) {Retropropagazione\\[2pt] $\delta_2 = (\delta W_3^{\top}) \odot (1-a_2^2)$\\[1pt] $\delta_1 = (\delta_2 W_2^{\top}) \odot (1-a_1^2)$};
...
\begin{tikzpicture}[
  ...
  % Colonna sinistra: batch, media, norme
  ...
  \node[box, below=of mean] (norms) {Norme per-esempio\\[2pt] a blocchi di 32: accumula $\sum_i\|g_i\|^2$};

  % Colonna destra: varianza e uscita
  \node[box, right=2.0cm of norms] (var) {Varianza\\[2pt] $V = \dfrac{\sum_i\|g_i\|^2 - n_k\|\bar g\|^2}{n_k - 1}$};
...
\begin{tikzpicture}[
  ...
  box/.style={
    draw=coverblue, fill=coverblue!6, rounded corners=3pt,
    text width=4.4cm, align=center, minimum height=1.2cm,
    inner sep=5pt
  },
  io/.style={
    draw=gray!60, fill=gray!8, rounded corners=3pt,
  ...
  % Riga superiore: ingresso e loss media
  \node[io] (in) {Sottocampione $\mathcal{H} \subseteq \mathcal{S}$\\[2pt] e vettore $v$};
  \node[box, right=2.0cm of in] (f) {Loss media sul sottocampione\\[2pt] $f(w') = J_\mathcal{H}(w')$};

  % Riga inferiore: Hessiano-vettore e uscita
  \node[box, below=of f] (hvp) {Differenziazione automatica\\[2pt] $\nabla^2 f(w)\,v$ (una sola chiamata)};
  \node[io, left=2.0cm of hvp] (out) {Vettore per il\\[2pt] gradiente coniugato};
