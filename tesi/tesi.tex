...
\begin{enumerate}
  ...
  Trattazione completa della dinamica del batch basata sul controllo della
  varianza, con convergenza lineare in aspettativa e stima di complessità
  totale competitiva con quella di SGD per problemi mal condizionati.
  \item \textbf{Estensione al metodo Newton-CG con sottocampionamento
  dell'Hessiana.} Metodo di secondo ordine che usa due campioni distinti per
  gradiente e Hessiana, con un criterio di arresto adattivo per il gradiente
  coniugato interno basato sulla varianza campionaria dei prodotti
  Hessiana-vettore.
  ...
  non liscio mediante gradiente generalizzato, active set e ricerca lineare
  proiettata; gli algoritmi sono inoltre implementati in un'applicazione web
  interattiva per la visualizzazione e la sperimentazione.
\end{enumerate}

\subsection{Struttura della tesi}

Il resto del lavoro è organizzato come segue. Il Capitolo~\ref{sec:background}
richiama prerequisiti e notazione; il Capitolo~\ref{sec:formulazione}
...
condizionamento. Le nozioni probabilistiche di base (valore atteso, varianza)
sono date per note.
...
\begin{notebox}
  ...
  \]
  dove il quadrato è inteso \emph{componente per componente}. Se i gradienti
  ...
  è il punto di partenza della \emph{Condizione di Controllo della Varianza}
...
I concetti introdotti in questo capitolo -- convessità, gradiente, norme,
condizionamento e varianza -- sono gli strumenti con cui il prossimo capitolo
formalizza il problema di ottimizzazione empirica su larga scala. In
