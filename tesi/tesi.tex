...
gradiente a campione dinamico sul problema quadratico ($\kappa \approx 1.1$)
...
\subsection{B.5: metodo RAPG (Robust Adaptive Preconditioned Gradient)}

Il metodo RAPG mantiene il nucleo BB-CCV (passo di Barzilai--Borwein con
campionamento dinamico CCV) e lo combina con un precondizionatore diagonale di
tipo RMSprop limitato, per accelerare la convergenza sui problemi mal
condizionati, e con una ricerca di radice in stile Regula Falsi per i problemi
in una dimensione. Il nome esteso del metodo è \emph{Robust Adaptive
Preconditioned Gradient} (gradiente adattivo precondizionato robusto).

\begin{pythoncode}[title= RAPG]

def rapg(w0, theta, max_iter, alpha, batch0):
    w = np.array(w0, dtype=float)
    N = max(len([grad_i(w, i) for i in range(max(batch0, 1))]), batch0)
...
Oltre allo stato del passo di Barzilai--Borwein ($w_{\text{prev}}$, $g_{\text{prev}}$)
si inizializza lo stato del precondizionatore: il momento secondo
$v \in \mathbb{R}^d$ (inizialmente nullo), il coefficiente $\beta_2 = 0.99$ e la
costante $\varepsilon = 10^{-10}$ che evita la divisione per zero. Per il caso
unidimensionale si inizializzano anche gli estremi del bracket della Regula
Falsi, $w_{\text{lo}}$, $g_{\text{lo}}$, $w_{\text{hi}}$, $g_{\text{hi}}$,
usati per intercettare uno zero del gradiente.
...
L'estrazione del batch e il passo di Barzilai--Borwein sono identici al
BB-CCV (Sezione~B.4): $\alpha_k^{\mathrm{BB}} = s_k^{\top}s_k / s_k^{\top}y_k$
salvaguardato in $[\alpha/20,\,5\alpha]$, con $\alpha_0 = \alpha$.
...
In una dimensione la direzione di ricerca non è il gradiente ma l'incremento
che porta all'annullamento del gradiente, cercato con il metodo della Regula
Falsi. Si mantiene un bracket $[w_{\text{lo}}, w_{\text{hi}}]$ ai cui estremi il
gradiente ha segno opposto ($g_{\text{lo}}\,g_{\text{hi}} < 0$); se non è ancora
disponibile, lo si costruisce provando il punto $w - \alpha\,\mathrm{sign}(g)$.
Lo zero approssimato del gradiente è il punto di intersezione della secante:
\[
w_{\text{new}} = \frac{w_{\text{lo}}\,g_{\text{hi}} - w_{\text{hi}}\,g_{\text{lo}}}
{g_{\text{hi}} - g_{\text{lo}}},
\]
e la direzione è $d = w_{\text{new}} - w$. Per robustezza la direzione è
salvaguardata: se $|d| > 20\alpha$ si usa la direzione di discesa del gradiente
$-g$. Il bracket viene poi aggiornato sostituendo l'estremo con segno di
$g$ concorde a quello di $g_{\text{lo}}$.
...
In più dimensioni la direzione è il gradiente precondizionato
$d_k = -P_k\,g_k\,\alpha_k$, dove $P_k$ è un precondizionatore diagonale di tipo
RMSprop limitato. Si aggiorna il momento secondo componente per componente,
\[
v_k = \beta_2\, v_{k-1} + (1-\beta_2)\, g_k^2,
\]
con correzione del bias $\hat{v}_k = v_k / (1-\beta_2^{k+1})$ (necessaria
perché $v$ parte da zero), e si definisce
\[
P_k = \mathrm{clip}\!\left(\frac{1}{\sqrt{\hat{v}_k} + \varepsilon},\, 0.1,\, 10\right),
\]
cioè l'inverso della deviazione standard stimata, limitato tra $0.1$ e $10$ per
stabilità. L'effetto è di ridimensionare le coordinate secondo la loro
variabilità: le componenti a gradiente molto variabile (tipicamente quelle del
problema mal condizionato) vengono accorciate, quelle a gradiente stabile
allungate, migliorando il numero di condizionamento effettivo.
...
La line search di Armijo con backtracking si applica alla direzione scelta:
nel caso unidimensionale si riduce lo scalare $\texttt{step\_size}$ che
moltiplica $d$ (condizione $J(w + t\,d) \le J(w) + c_1 t\, g^{\top}d$), nel caso
multidimensionale si riduce direttamente la lunghezza di $d$ (condizione
$J(w + d) \le J(w) - c_1 d^{\top}g$). Dopo 20 (1D) o 30 (2D) dimezzamenti senza
successo si accetta un passo nullo. L'aggiornamento è $w_{k+1} = w_k + d_k$.
...
La gestione del batch è la CCV del Dynamic GD (Sezione~B.1): varianza
campionaria $\widehat{\mathcal{V}}$, norma $\|\widehat{\mathcal{V}}\|_1$ e
aggiornamento $n \leftarrow \min\{N,\ \lceil \|\widehat{\mathcal{V}}\|_1/(\theta^2\|g_k\|_2^2)\rceil + 1\}$
quando $\|\widehat{\mathcal{V}}\|_1/n_k > \theta^2\|g_k\|_2^2$. Come per gli altri
metodi, si registrano $w_{k+1}$ e $n_k$, si verifica la convergenza su
$\|\nabla J(w_{k+1})\|_2 < 10^{-6}$ e si restituiscono \texttt{history} e
\texttt{batch\_sizes}.
...
% ═══════════════════════════════════════════════════════════════════════════
\section{Schemi degli algoritmi BB-CCV e RAPG}\label{app:bbccv}
% ═══════════════════════════════════════════════════════════════════════════

Questa appendice riporta gli schemi a blocchi dei due metodi aggiuntivi usati
per i test comparativi dell'Appendice~\ref{app:test}: BB-CCV (gradiente a
campione dinamico con passo di Barzilai--Borwein) e RAPG, che ne estende il
nucleo con un precondizionatore diagonale.
...
% Schema E.2 - RAPG (Robust Adaptive Preconditioned Gradient)
\begin{figure}[H]
\centering
\resizebox{0.85\textwidth}{!}{%
\begin{tikzpicture}[
  ...
  % Nodi principali (colonna centrale)
  \node[io] (init) {Inizializzazione\\[2pt] $w_0$, $\mathcal{S}_0$, $\theta$, $\beta_2$};
  \node[box, below=of init] (batch) {Estrazione batch\\[2pt] $\mathcal{S}_k \subseteq \{1,\dots,N\}$, $|\mathcal{S}_k| = n_k$};
  \node[box, below=of batch] (grad) {Calcolo gradiente\\[2pt] $g_k = \nabla J_{\mathcal{S}_k}(w_k)$\\[1pt] e varianza campionaria $\widehat{\mathcal{V}}$};
  \node[box, below=of grad] (ccv) {Verifica CCV\\[3pt] $\displaystyle\frac{\|\widehat{\mathcal{V}}\|_1}{n_k} \le \theta^2 \|g_k\|^2$};
  \node[box, below=of ccv, text width=5.9cm, minimum height=1.9cm] (bb) {Passo di Barzilai-Borwein\\[2pt] $s = w_k - w_{k-1}$, \; $y = g_k - g_{k-1}$\\[2pt] $\alpha_k = \mathrm{clip}\!\Bigl(\dfrac{s^T s}{s^T y}, \dfrac{\alpha}{20}, 5\alpha\Bigr)$};
  \node[box, below=of bb, text width=6.2cm, minimum height=2.6cm] (prec) {Precondizionatore diagonale (RMSprop limitato)\\[2pt] $v_k = \beta_2 v_{k-1} + (1-\beta_2) g_k^2$\\[2pt] $P_k = \mathrm{clip}\!\Bigl(\dfrac{1}{\sqrt{\hat{v}_k} + \varepsilon},\, 0.1,\, 10\Bigr)$\\[2pt] $d_k = -P_k g_k \alpha_k$};
  \node[box, below=of prec] (ls) {Line search di Armijo\\[2pt] backtracking su $d_k$};
  \node[box, below=of ls] (upd) {Aggiornamento\\[2pt] $w_{k+1} = w_k + d_k$};
  \node[io, below=of upd] (conv) {Convergenza?\\[2pt] $\|\nabla J(w_k)\| < \mathrm{tol}$};
  \node[io, right=2.1cm of conv] (end) {Fine:\\[2pt] $w^* \approx w_k$};
...
\end{tikzpicture}}
\caption{Schema dell'algoritmo \emph{RAPG} (Robust Adaptive Preconditioned
Gradient). Il flusso è quello del BB-CCV (Figura~\ref{fig:bbccv}), con la CCV
per il campionamento dinamico del batch e il passo di Barzilai--Borwein; la
differenza è nella direzione di ricerca: in più dimensioni il gradiente è
precondizionato dalla diagonale $P_k$, costruita come inverso della deviazione
standard stimata dei gradienti (momento secondo $v_k$ con $\beta_2 = 0.99$ e
correzione del bias $\hat{v}_k = v_k/(1-\beta_2^{k+1})$), limitata tra $0.1$ e
$10$; in una dimensione la direzione è invece ottenuta con una ricerca di
radice in stile Regula Falsi sul gradiente (Sezione~B.5). Il precondizionatore
riduce il numero di condizionamento effettivo del problema, accelerando la
convergenza sui problemi mal condizionati.}
\label{fig:rapg}
\end{figure}


% ═══════════════════════════════════════════════════════════════════════════
\section{Test comparativi aggiuntivi}\label{app:test}

Questa appendice raccoglie i test comparativi eseguiti con il pannello
``Analisi'' dell'applicazione interattiva (Sezione~\ref{sec:visualizzazione})
sui cinque algoritmi dell'applicazione: Dynamic GD, Newton-CG,
Newton-CG~$L_1$, RAPG e BB-CCV (Appendice~\ref{app:bbccv}). Per ogni
iterazione il pannello riporta il numero di iterazione, l'errore
$\|w_k - w_*\|_2$ e il valore $J(w_k)$. I test sono stati eseguiti sulle tre
funzioni obiettivo quadratiche dell'applicazione: ben condizionata
($\kappa \approx 1.1$), molto mal condizionata ($\kappa \approx 100$) e con
termine incrociato. Le Figure~F.1--F.3 mostrano le schermate del pannello,
raggruppate per funzione obiettivo, con i cinque algoritmi in ordine:
Dynamic GD, Newton-CG, Newton-CG~$L_1$, RAPG, BB-CCV.

% Figura F.1 - quadratica ben condizionata
\begin{figure}[H]
\centering
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/bencond_dgd.png}
  \caption{Dynamic GD}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/bencond_ncg.png}
  \caption{Newton-CG}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/bencond_ncgl1.png}
  \caption{Newton-CG $L_1$}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/bencond_rapg.png}
  \caption{RAPG}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/bencond_bbccv.png}
  \caption{BB-CCV}
\end{subfigure}
\caption{Pannello di analisi dell'applicazione sul problema quadratico
ben condizionato ($\kappa \approx 1.1$): per ogni iterazione sono riportati
numero di iterazione, $\|w_k - w_*\|_2$ e $J(w_k)$, per i cinque algoritmi
(a) Dynamic GD, (b) Newton-CG, (c) Newton-CG~$L_1$, (d) RAPG, (e) BB-CCV.}
\label{fig:test_bencond}
\end{figure}

% Figura F.2 - quadratica molto mal condizionata
\begin{figure}[H]
\centering
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/malcond_dgd.png}
  \caption{Dynamic GD}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/malcond_ncg.png}
  \caption{Newton-CG}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/malcond_ncgl1.png}
  \caption{Newton-CG $L_1$}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/malcond_rapg.png}
  \caption{RAPG}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/malcond_bbccv.png}
  \caption{BB-CCV}
\end{subfigure}
\caption{Pannello di analisi dell'applicazione sul problema quadratico
molto mal condizionato ($\kappa \approx 100$): per ogni iterazione sono
riportati numero di iterazione, $\|w_k - w_*\|_2$ e $J(w_k)$, per i cinque
algoritmi (a) Dynamic GD, (b) Newton-CG, (c) Newton-CG~$L_1$, (d) RAPG,
(e) BB-CCV.}
\label{fig:test_malcond}
\end{figure}

% Figura F.3 - quadratica con termine incrociato
\begin{figure}[H]
\centering
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/incrociato_dgd.png}
  \caption{Dynamic GD}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/incrociato_ncg.png}
  \caption{Newton-CG}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/incrociato_ncgl1.png}
  \caption{Newton-CG $L_1$}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/incrociato_rapg.png}
  \caption{RAPG}
\end{subfigure}\hfill
\begin{subfigure}{0.19\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figure_test/incrociato_bbccv.png}
  \caption{BB-CCV}
\end{subfigure}
\caption{Pannello di analisi dell'applicazione sul problema quadratico
con termine incrociato: per ogni iterazione sono riportati numero di
iterazione, $\|w_k - w_*\|_2$ e $J(w_k)$, per i cinque algoritmi
(a) Dynamic GD, (b) Newton-CG, (c) Newton-CG~$L_1$, (d) RAPG, (e) BB-CCV.}
\label{fig:test_incrociato}
\end{figure}

\paragraph{Osservazioni comparative.} Dalle schermate emergono le seguenti
...
iterazioni): il più veloce è Dynamic GD ($\approx 10^{-1}$) insieme a
Newton-CG~$L_1$, mentre Newton-CG è il più lento ($\approx 4\times10^{-1}$).
Sul problema molto mal condizionato il quadro cambia drasticamente: Dynamic
GD, Newton-CG~$L_1$ e RAPG restano bloccati sopra $10^{-1}$, mentre BB-CCV
converge alla precisione macchina ($\|w-w_*\| \approx 10^{-14}$) in appena 10
iterazioni. Il passo di Barzilai--Borwein stima la curvatura usando solo i
gradienti già calcolati e non richiede la Hessiana: è particolarmente
efficace proprio sui problemi mal condizionati, in accordo con
l'Appendice~E. Sul problema con termine incrociato BB-CCV è di nuovo il
migliore ($\approx 6\times10^{-4}$ in 16 iterazioni), seguito da Dynamic GD
($\approx 10^{-2}$) e Newton-CG~$L_1$ ($\approx 10^{-2}$); Newton-CG e RAPG
restano attorno a $10^{-1}$. In sintesi, BB-CCV è il metodo che performa
meglio in assoluto sui problemi considerati, in particolare su quelli mal
condizionati; Dynamic GD è competitivo sui problemi ben condizionati e con
termine incrociato ma collassa su quelli molto mal condizionati, dove solo
l'informazione di curvatura -- esplicita in Newton-CG o stimata dal passo di
Barzilai--Borwein in BB-CCV -- permette di progredire.
