...
soglia. In questo modo la non differenziabilità della $L_1$ in $0$ viene
aggirata senza mai derivare il termine non liscio: l'operatore $S_\nu(w_i)$ è
la soluzione esatta in forma chiusa del problema $\min_z \tfrac{1}{2}(z-w_i)^2
+ \nu|z|$, la cui condizione di ottimalità --- scritta con la sottoderivata
della norma, che in $z=0$ vale $[-\nu,\nu]$ --- si risolve analiticamente: per
$|w_i| \le \nu$ il minimizzatore è $z=0$ (il punto di non differenziabilità è
un minimo stazionario), per $|w_i| > \nu$ è
$\operatorname{sign}(w_i)(|w_i|-\nu)$. Il passo di discesa coinvolge quindi
solo la parte liscia $J$; il termine $L_1$ non viene derivato né trascurato,
ma minimizzato esattamente. In questa tesi si segue invece la strategia
