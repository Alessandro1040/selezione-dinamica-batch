...
Nel \emph{machine learning} su larga scala si dispone di un dataset di $N$
...
Questo capitolo richiama la notazione usata nel resto del lavoro. Le nozioni
matematiche di base (convessità, gradiente, Hessiana, norme, valore atteso e
varianza) sono date per note.
...
$\nabla J(w)$ & gradiente di $J$ \\
$\nabla^2 J(w)$ & Hessiana di $J$ \\
$\lambda, L$ & costanti di convessità forte e di Lipschitz del gradiente \\
$\kappa = L/\lambda$ & numero di condizionamento \\
$\theta \in (0,1)$ & tolleranza della condizione di controllo della varianza \\
$\alpha_k$ & passo all'iterazione $k$ \\
$\mathcal{V}$ & varianza della popolazione, componente per componente \\
